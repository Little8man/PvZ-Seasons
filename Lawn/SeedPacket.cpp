...
void SeedPacketDrawSeed(Graphics* g, float x, float y, SeedType theSeedType, SeedType theImitaterType, float theOffsetX, float theOffsetY, float theScale)
{
	...
	if ((aSeedType == SeedType::SEED_POTATOMINE || aSeedType == SeedType::SEED_POPCORN) && g->mScaleX <= 1.0f)
	{
	...
	else if ((aSeedType == SeedType::SEED_TALLNUT || aSeedType == SeedType::SEED_VINENUT) && g->mScaleX <= 1.0f)
	{
...
void DrawSeedPacket(Graphics* g, float x, float y, SeedType theSeedType, SeedType theImitaterType, float thePercentDark, int theGrayness, bool theDrawCost, bool theUseCurrentCost)
{
	...
	{
		SexyString aCostStr;
		float aDrawCostMultiplier = gLawnApp->GetConfigCheckbox(false, CONFIG_CHECKBOX_SUN_AMOUNT) ? 12.5 : 1;
		if (gLawnApp->mBoard && (gLawnApp->mBoard->PlantUsesAcceleratedPricing(aSeedType) || aSeedType == SeedType::SEED_COFFEEPOT || gLawnApp->IsAcceleratedLevel()))
		{
			if (theUseCurrentCost)
			{
				aCostStr = StrFormat(_S("%d"), (int)(gLawnApp->mBoard->GetCurrentPlantCost(theSeedType, theImitaterType) * aDrawCostMultiplier));
			}
			else
			{
				aCostStr = StrFormat(_S("%d+"), (int)(Plant::GetCost(theSeedType, theImitaterType) * aDrawCostMultiplier));
			}
		}
		else
		{
...
void SeedPacket::MouseDown(int x, int y, int theClickCount)
{
	...
	if (!mApp->mEasyPlantingCheat)
	{
		if (!mActive)
		{
			...
			if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == DAY_LEVEL_INDEX + 1 && mBoard->mHelpDisplayed[(int)AdviceType::ADVICE_CLICK_ON_SUN])
			{
		...
		{
			mApp->PlaySample(SOUND_BUZZER);
			if (aUseSeedType == SeedType::SEED_COBCANNON)
			{
...
void SeedPacket::WasPlanted()
{
	...
	else if (mApp->IsLastStandLevel() && mBoard->mChallenge->mChallengeState != ChallengeState::STATECHALLENGE_LAST_STAND_ONSLAUGHT)
	{
		mTimesUsed++;
		mActive = true;
		FlashIfReady();
...
void SeedBank::Draw(Graphics* g)
{
	...
	if (!mBoard->HasConveyorBeltSeedBank())
	{
		...
		SexyString aMoneyLabel = StrFormat(_S("%d"), max((int)(mBoard->mSunMoney * aDrawCostMultiplier), 0));
	...
	if (mBoard->mShadowSunMoney > 0)
	{
		SexyString aShadowSunLabel = StrFormat(_S("%d"), max((int)(mBoard->mShadowSunMoney * aDrawCostMultiplier), 0));
		Color aShadowSunColor(0, 0, 0);

		g->DrawImage(IMAGE_SHADOW_SUN_BANK, -80, 0);
		TodDrawString(g, aShadowSunLabel, -46, 78, FONT_CONTINUUMBOLD14, aShadowSunColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
...
void SeedPacket::SetPacketType(SeedType theSeedType, SeedType theImitaterType)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM || mApp->IsLastStandLevel() ||
	...
	if (((Plant::IsUpgrade(aUseSeedType) || Plant::IsPremium(aUseSeedType)) && !gLawnApp->IsSurvivalMode()) || Plant::GetRefreshTime(mPacketType, mImitaterType) == 5000)
	{
		mRefreshTime = 3500;
		mRefreshing = true;
		mActive = false;
	}
	else if (aUseSeedType == SeedType::SEED_POPCORN)
	{
		mRefreshTime = 3000;
		mRefreshing = true;
		mActive = false;
	}
	else if ((Plant::IsUpgrade(aUseSeedType) || Plant::IsPremium(aUseSeedType)) && gLawnApp->IsSurvivalMode())
	{
		mRefreshTime = 8000;
		mRefreshing = true;
		mActive = false;
