...
SeedChooserScreen::SeedChooserScreen()
{
	...
	mLeftButton->SetLabel(_S("LEFT"));
	mLeftButton->mButtonImage = Sexy::IMAGE_SEEDCHOOSER_BUTTON;
	...
	mLeftButton->mOverOverlayImage = Sexy::IMAGE_SEEDCHOOSER_BUTTON_GLOW;
	mLeftButton->SetFont(Sexy::FONT_DWARVENTODCRAFT15);
	mLeftButton->Resize(SEED_CHOOSER_PLAYBUTTON_X - 234 - 27, SEED_CHOOSER_PLAYBUTTON_Y, 156, 42);
	mLeftButton->mTextOffsetY = -1;
	...
	mRightButton->SetLabel(_S("RIGHT"));
	mRightButton->mButtonImage = Sexy::IMAGE_SEEDCHOOSER_BUTTON;
	...
	mRightButton->mOverOverlayImage = Sexy::IMAGE_SEEDCHOOSER_BUTTON_GLOW;
	mRightButton->SetFont(Sexy::FONT_DWARVENTODCRAFT15);
	mRightButton->Resize(SEED_CHOOSER_PLAYBUTTON_X + 78 + 27, SEED_CHOOSER_PLAYBUTTON_Y, 156, 42);
	mRightButton->mTextOffsetY = -1;
	...
	mStartButton->Resize(SEED_CHOOSER_PLAYBUTTON_X - 78, SEED_CHOOSER_PLAYBUTTON_Y, 156, 42);
...
void SeedChooserScreen::CrazyDavePickSeeds()
{
	...
	for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER; aSeedType = (SeedType)(aSeedType + 1))
	{
		...
		if (!mApp->SeedTypeAvailable(aSeedType) || aRecFlags || SeedNotAllowedToPick(aSeedType) || 
			Plant::IsUpgrade(aSeedType) || Plant::IsPremium(aSeedType) || aSeedType == SEED_IMITATER || aSeedType == SEED_UMBRELLA || aSeedType == SEED_BLOVER || gPlantDefs[aSeedType].mPage < 0)
		{
	...
	if (mBoard->mZombieAllowed[ZOMBIE_BUNGEE] || mBoard->mZombieAllowed[ZOMBIE_CATAPULT] || mBoard->mZombieAllowed[ZOMBIE_GIGABUNGEE] || mBoard->mZombieAllowed[ZOMBIE_GIGACATAPULT])
	{
		aSeedArray[SEED_UMBRELLA].mWeight = 1;
	}
	if (mBoard->mZombieAllowed[ZOMBIE_BALLOON] || mBoard->mZombieAllowed[ZOMBIE_GIGABALLOON] || mBoard->StageHasFog())
	{
...
void SeedChooserScreen::GetSeedPositionInChooser(int theIndex, int& x, int& y)
{
	...
	else
	{
		...
		int aRow = theRealIndex / 16;
		int aCol = theRealIndex % 16;
...
{
	return mApp->IsLastStandLevel() && theSeedType != SEED_IMITATER &&
		(theSeedType == SeedType::SEED_GOLDBLOOM || theSeedType == SeedType::SEED_STUFFYSUNFLOWER || theSeedType == SeedType::SEED_MARIGOLD || Plant::MakesSun(theSeedType) || Plant::GetCost(theSeedType, theSeedType) == 0);
}
...
void SeedChooserScreen::Draw(Graphics* g)
{
	...
	TodDrawString(g, _S("[CHOOSE_YOUR_SEEDS]"), SEED_CHOOSER_TEXT_X, SEED_CHOOSER_TEXT_Y, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White, DS_ALIGN_CENTER);
	...
	for each (auto plantDefinition in gPlantDefs)
	{
		if (true && plantDefinition.mPage == mPage) {
			SeedType aSeedType = plantDefinition.mSeedType;
	...
	for each (auto plantDefinition in gPlantDefs)
	{
		if (true) {
				...
				aSeedState != SEED_PACKET_HIDDEN && (aSeedState == SEED_IN_CHOOSER || mBoard->mCutScene->mSeedChoosing))
			{
				...
				if (aSeedState == SEED_IN_BANK || (aSeedState != SEED_IN_BANK && plantDefinition.mPage == mPage))
...
int SeedChooserScreen::FlyersAreComming()
{
	for (int aWave = 0; aWave < mBoard->mNumWaves; aWave++)
	{
		for (int anIndex = 0; anIndex < MAX_ZOMBIES_IN_WAVE; anIndex++)
		{
			...
			if (Zombie::IsBalloon(aZombieType))
				return true;
		}
	}
	return false;
...
void SeedChooserScreen::OnStartButton()
{
	...
	if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == NIGHT_LEVEL_INDEX + 1 && !PickedPlantType(SEED_PUFFSHROOM))
	{
	...
	if (!PickedPlantType(SEED_SUNFLOWER) && 
		!PickedPlantType(SEED_TWINSUNFLOWER) && 
		!PickedPlantType(SEED_SUNSHROOM) &&
		!PickedPlantType(SEED_PRIMALSUNFLOWER) &&
		!PickedPlantType(SEED_POWERFLOWER) &&
		!mBoard->mCutScene->IsSurvivalRepick() && !mApp->IsLastStandLevel())
	{
		if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == NIGHT_LEVEL_INDEX + 1)
		{
	...
	if (mBoard->StageHasWater() && !PickedPlantType(SEED_LILYPAD) && !PickedPlantType(SEED_SEASHROOM) && !PickedPlantType(SEED_TANGLEKELP) && !PickedPlantType(SEED_CATTAIL) && !mBoard->mCutScene->IsSurvivalRepick())
	{
		if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == POOL_LEVEL_INDEX + 1)
		{
	...
	if (mBoard->StageHasTiles() && !PickedPlantType(SEED_FLOWERPOT) && mApp->SeedTypeAvailable(SEED_FLOWERPOT))
	{
	...
	if (!CheckSeedUpgrade(SEED_COBCANNON, SEED_KERNELPULT))
		return;
...
void SeedChooserScreen::ButtonDepress(int theId)
{
	...
	{
		if (mPage < SeedChooserPage::NUM_PAGES - 1)
			mPage++;
	}
	else if (theId == SeedChooserScreen::SeedChooserScreen_Left)
	{
		int aMinPage = mApp->mTodCheatKeys ? SeedChooserPage::PAGE_DEBUG : PAGE_NORMAL;
		if (mPage > aMinPage)
			mPage--;
	}
	else if (theId == SeedChooserScreen::SeedChooserScreen_Store)
	{
		...
		else if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE)) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_LAWNDEFENSEDILEMNA);
		else mApp->mMusic->MakeSureMusicIsPlaying(MUSIC_TUNE_CHOOSE_YOUR_SEEDS);
	}
...
SeedType SeedChooserScreen::SeedHitTest(int x, int y)
{
	if (mMouseVisible)
	{
		for each (auto plantDefinition in gPlantDefs)
		{
			...
			if (true && (aChosenSeed.mSeedState == SEED_IN_BANK) || plantDefinition.mPage == mPage) {
				SeedType aSeedType = plantDefinition.mSeedType;
...
void SeedChooserScreen::MouseDown(int x, int y, int theClickCount)
{
	...
	else
	{
		if (!mBoard->mSeedBank->ContainsPoint(x, y) && !mAlmanacButton->IsMouseOver() && !mStoreButton->IsMouseOver() && mApp->CanShowAlmanac())
		{
			...
			if (aZombie && aZombie->mFromWave == Zombie::ZOMBIE_WAVE_CUTSCENE && aZombie->mZombieType != ZOMBIE_GIGAGARGANTUAR)
			{
