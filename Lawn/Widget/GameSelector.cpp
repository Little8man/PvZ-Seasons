...
GameSelector::GameSelector(LawnApp* theApp)
{
	...
	mOptionsButton->Resize(0, 0, Sexy::IMAGE_SELECTORSCREEN_OPTIONS1->mWidth, Sexy::IMAGE_SELECTORSCREEN_OPTIONS1->mHeight + 33);
	...
	if (mApp->mPlayerInfo->mConfigCheckbox[ConfigCheckbox::CONFIG_CHECKBOX_GIGA_MODE]) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_ZOMBOTANY);
...
void GameSelector::ButtonDepress(int theId)
{
	...
	case GameSelector::GameSelector_Help:
		mApp->KillGameSelector();
		mApp->ShowConfigScreen(ConfigPage::CONFIG_PAGE_TEMP);
	...
	case GameSelector::GameSelector_Store:
	{
		...
		if (aStore->mGoToTreeNow)
		{
			...
			mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_ZOMBOTANY);
		else 
...
void GameSelector::AddPreviewProfiles()
{
	...
	if (aProfile)
	{
		aProfile->mLevel = NIGHT_LEVEL_INDEX + 1;
	...
	if (aProfile)
	{
		aProfile->mLevel = POOL_LEVEL_INDEX + 1;
	...
	if (aProfile)
	{
		aProfile->mLevel = FOG_LEVEL_INDEX + 1;
		aProfile->mHasUnlockedMinigames = 1;
	...
	if (aProfile)
	{
		aProfile->mLevel = ROOF_LEVEL_INDEX + 1;
		aProfile->mHasUnlockedMinigames = 1;
	...
	aProfile = mApp->mProfileMgr->AddProfile(_S("6 Moon"));
	if (aProfile)
	{
		aProfile->mLevel = MOON_LEVEL_INDEX + 1;
		aProfile->mHasUnlockedMinigames = 1;
		aProfile->mHasUnlockedPuzzleMode = 1;
		aProfile->mHasUnlockedSurvivalMode = 1;
		aProfile->mPurchases[StoreItem::STORE_ITEM_PACKET_UPGRADE] = 2;
