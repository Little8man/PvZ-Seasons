...
void StoreScreen::Update()
{
    ...
    else mApp->mMusic->MakeSureMusicIsPlaying(MUSIC_TUNE_TITLE_CRAZY_DAVE_MAIN_THEME);
...
bool StoreScreen::IsPageShown(StorePages thePage)
{
    ...
    if (thePage == STORE_PAGE_PLANT_UPGRADES) return mApp->mPlayerInfo->mLevel >= ROOF_LEVEL_INDEX + 2;
    // 到达或已通过冒险模式 5-5 关卡时，显示花园工具页
    if (thePage == STORE_PAGE_ZEN1) return mApp->mPlayerInfo->mLevel >= ROOF_LEVEL_INDEX + 5;
    // 冒险模式未完成时，不显示智慧树工具页
