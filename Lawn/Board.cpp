...
Board::Board(LawnApp* theApp)
{
	...
	// U53RDV
	mDevMode = mApp->mDevMode;
	// Fast Forward
	mFastForwardButton = new GameButton(2);
	mFastForwardButton->mButtonImage = Sexy::IMAGE_BUTTON_FASTFORWARD;
	mFastForwardButton->mDownImage = Sexy::IMAGE_BUTTON_FASTFORWARD_DOWN;
	mFastForwardButton->Resize(FASTFORWARD_BUTTON_X, FASTFORWARD_BUTTON_Y, 46, 46);
	mFastForward = false;
	// Config
	for (int i; i < NUM_CONFIG_CHECKBOXES; i++)
	{
		mConfigCheckbox[i] = mApp->mPlayerInfo->mConfigCheckbox[i];
	}

	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		mMenuButton->SetLabel(_S("[MAIN_MENU_BUTTON]"));
		mMenuButton->Resize(MAIN_MENU_BUTTON_X, MENU_BUTTON_Y, 163, 46);
	...
	else
	{
		...
		mMenuButton->Resize(MENU_BUTTON_X, MENU_BUTTON_Y, 117, 46);
	}
...
Board::~Board()
{
	...
	if (mFastForwardButton)
	{
		delete mFastForwardButton;
	}
	if (mStoreButton)
	{
		delete mStoreButton;
...
{
	if (mApp->IsFirstTimeAdventureMode() && mLevel == DAY_LEVEL_INDEX + 1)
		return false;
...
void Board::PickZombieWaves()
{
	...
	if (mApp->IsAdventureMode())
	{
		...
		{
			mNumWaves = gZombieWaves[ClampInt(mLevel - 1, 0, FINAL_LEVEL - 1)];
			if (!mApp->IsFirstTimeAdventureMode() && !mApp->IsMiniBossLevel())
			{
				mNumWaves = mNumWaves < 10 ? 20 : mNumWaves + (10 * mApp->IsFirstTimeAdventureMode());
			}
		}
	}
	else
	{
		...
		if (mApp->IsSurvivalMode() || mApp->IsLastStandLevel())
			...
			|| mApp->IsPortalLevel() || aGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL || aGameMode == GameMode::GAMEMODE_CHALLENGE_ECLIPSE
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_LOOKUPPVZINFLATION || aGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_DAY
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_NIGHT || aGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_FOG
			)
			mNumWaves = 20;
		else if (mApp->IsStormyNightLevel() || mApp->IsLittleTroubleLevel() || mApp->IsBungeeBlitzLevel() 
			|| mApp->IsColumnLevel() || mApp->IsShovelLevel() || aGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_2
			||	aGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 || aGameMode == GameMode::GAMEMODE_CHALLENGE_POGO_PARTY
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_SHALLNOTPASS || aGameMode == GameMode::GAMEMODE_CHALLENGE_HEAVEN || aGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBOWLING
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_DEADPLANTS || aGameMode == GameMode::GAMEMODE_CHALLENGE_CUBELAND
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_POOL || aGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_ROOF
			)
			mNumWaves = 30;
		else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_DAY || aGameMode == GameMode::GAMEMODE_CHALLENGE_NIGHT
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_POOL ||aGameMode == GameMode::GAMEMODE_CHALLENGE_FOG
			|| aGameMode == GameMode::GAMEMODE_CHALLENGE_ROOF || aGameMode == GameMode::GAMEMODE_CHALLENGE_MOON
			)
			mNumWaves = 50;
		else
			mNumWaves = 40;
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		else if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != BOWLING_LEVEL)
		{
		...
		if (aIsFlagWave)
		{
			...
			if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SUNNY_DAY)
			{
				...
				{
					PutZombieInWave(ZombieType::ZOMBIE_MUSTACHE, aWave, &aZombiePicker);
				}
				PutZombieInWave(ZombieType::ZOMBIE_RALLY, aWave, &aZombiePicker);
			}
			else if (mApp->IsZombotanyLevel())
			{
				for (int _i = 0; _i < aPlainZombiesNum; _i++)
				{
					PutZombieInWave(ZombieType::ZOMBIE_PEA_HEAD, aWave, &aZombiePicker);
				}
			}
			else if (mApp->IsMinecraftLevel())
			{
				for (int _i = 0; _i < aPlainZombiesNum; _i++)
				{
					PutZombieInWave(ZombieType::MC_ZOMBIE, aWave, &aZombiePicker);
				}
			}
			else
			{
		...
		// 部分关卡的多倍出怪
		if (mApp->IsColumnLevel())
			aZombiePoints *= 6;
		else if (mApp->mGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_WALLNUT_BOWLING)
			aZombiePoints *= 2;
		else if (mApp->IsLittleTroubleLevel() || mApp->IsWallnutBowlingLevel())
			aZombiePoints *= 4;
		else if (mApp->IsMiniBossLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_HEAVEN)
			aZombiePoints *= 3;
		else if (mApp->IsShovelLevel() 
			|| mApp->IsBungeeBlitzLevel()
			|| mApp->IsSkipAreaLevel()
			|| mApp->IsPortalLevel()
			|| mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL
			|| mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_DEADPLANTS
			|| mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SUNNY_DAY)
			aZombiePoints *= 2;
		...
		if (aIntroZombieType != ZombieType::ZOMBIE_INVALID && aIntroZombieType != ZombieType::ZOMBIE_DUCKY_TUBE)
		{
			...
			if ((Zombie::IsDigger(aIntroZombieType) || Zombie::IsBalloon(aIntroZombieType)))
			{
			...
			else if (Zombie::IsYeti(aIntroZombieType))
			{
		...
		if (mLevel == FINAL_LEVEL && aIsFinalWave)
		{
...
void Board::LoadBackgroundImages()
{
	...
	case BackgroundType::BACKGROUND_1_DAY:
		...
		if ((mApp->IsAdventureMode() && mLevel <= DAY_LEVEL_INDEX + 4) || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED)
			TodLoadResources("DelayLoad_BackgroundUnsodded");
		if (mApp->IsMinecraftLevel())
			TodLoadResources("DelayLoad_BackgroundM");
		break;

	case BackgroundType::BACKGROUND_DAY_FALL:
		TodLoadResources("DelayLoad_BackgroundE");
		break;

	case BackgroundType::BACKGROUND_DAY_WINTER:
		TodLoadResources("DelayLoad_Background1W");
		TodLoadResources("DelayLoad_Background1");
		break;
	...
	case BackgroundType::BACKGROUND_NIGHT_FALL:
	...
	case BackgroundType::BACKGROUND_FOG_SPRING:
		...
		TodLoadResources("DelayLoad_BackgroundFog");
		break;
	...
	case BackgroundType::BACKGROUND_RESORT:
		TodLoadResources("DelayLoad_BackgroundR");
		break;
	...
	case BackgroundType::BACKGROUND_DEADPLANTS:
		TodLoadResources("DelayLoad_BackgroundD");
		TodLoadResources("DelayLoad_BackgroundFog");
		break;

	case BackgroundType::BACKGROUND_HEAVEN:
		TodLoadResources("DelayLoad_Background1");
		TodLoadResources("DelayLoad_BackgroundH");
		break;
...
void Board::PickBackground()
{
	...
	case GameMode::GAMEMODE_ADVENTURE:
		...
		else if (mLevel > DAY_LEVEL_INDEX && mLevel <= DAY_LEVEL_INDEX + LEVELS_PER_AREA)
			mBackground = BackgroundType::BACKGROUND_1_DAY;
		else if (mLevel > NIGHT_LEVEL_INDEX && mLevel <= NIGHT_LEVEL_INDEX + LEVELS_PER_AREA)
			mBackground = BackgroundType::BACKGROUND_2_NIGHT;
		else if (mLevel > POOL_LEVEL_INDEX && mLevel <= POOL_LEVEL_INDEX + LEVELS_PER_AREA)
			mBackground = BackgroundType::BACKGROUND_3_POOL;
		else if (mLevel > FOG_LEVEL_INDEX && mLevel <= FOG_LEVEL_INDEX + LEVELS_PER_AREA)
			mBackground = BackgroundType::BACKGROUND_4_FOG;
		else if (mLevel > ROOF_LEVEL_INDEX && mLevel <= ROOF_LEVEL_INDEX + LEVELS_PER_AREA)
			mBackground = BackgroundType::BACKGROUND_5_ROOF;
		else if ((mLevel > MOON_LEVEL_INDEX && mLevel <= MOON_LEVEL_INDEX + LEVELS_PER_AREA) || mApp->IsFinalBossLevel())
			mBackground = BackgroundType::BACKGROUND_6_BOSS;
		else if (mLevel > DAY_FALL_LEVEL_INDEX && mLevel <= DAY_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_DAY_FALL;
		else if (mLevel > NIGHT_FALL_LEVEL_INDEX && mLevel <= NIGHT_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_NIGHT_FALL;
		else if (mLevel > POOL_FALL_LEVEL_INDEX && mLevel <= POOL_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_FALL;
		else if (mLevel > FOG_FALL_LEVEL_INDEX && mLevel <= FOG_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_FALL;
		else if (mLevel > ROOF_FALL_LEVEL_INDEX && mLevel <= ROOF_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_FALL;
		else if (mLevel > MOON_FALL_LEVEL_INDEX && mLevel <= MOON_FALL_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_FALL;
		else if (mLevel > DAY_WINTER_LEVEL_INDEX && mLevel <= DAY_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_DAY_WINTER;
		else if (mLevel > NIGHT_WINTER_LEVEL_INDEX && mLevel <= NIGHT_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_NIGHT_WINTER;
		else if (mLevel > POOL_WINTER_LEVEL_INDEX && mLevel <= POOL_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_WINTER;
		else if (mLevel > FOG_WINTER_LEVEL_INDEX && mLevel <= FOG_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_WINTER;
		else if (mLevel > ROOF_WINTER_LEVEL_INDEX && mLevel <= ROOF_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_WINTER;
		else if (mLevel > MOON_WINTER_LEVEL_INDEX && mLevel <= MOON_WINTER_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_WINTER;
		else if (mLevel > DAY_SPRING_LEVEL_INDEX && mLevel <= DAY_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_DAY_SPRING;
		else if (mLevel > NIGHT_SPRING_LEVEL_INDEX && mLevel <= NIGHT_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_NIGHT_SPRING;
		else if (mLevel > POOL_SPRING_LEVEL_INDEX && mLevel <= POOL_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_SPRING;
		else if (mLevel > FOG_SPRING_LEVEL_INDEX && mLevel <= FOG_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_SPRING;
		else if (mLevel > ROOF_SPRING_LEVEL_INDEX && mLevel <= ROOF_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_POOL_SPRING;
		else if (mLevel > MOON_SPRING_LEVEL_INDEX && mLevel <= MOON_SPRING_LEVEL_INDEX + LEVEL_MID_AREA)
			mBackground = BackgroundType::BACKGROUND_FOG_SPRING;
		else
			mBackground = BackgroundType::BACKGROUND_1_DAY;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_SQUIRREL:
		// New
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_WALLNUT_BOWLING:
	case GameMode::GAMEMODE_CHALLENGE_MINIBOSS_DAY:
	case GameMode::GAMEMODE_CHALLENGE_DAY:
	case GameMode::GAMEMODE_CHALLENGE_ZOMBOWLING:
	case GameMode::GAMEMODE_CHALLENGE_LOOKUPPVZINFLATION:
		mBackground = BackgroundType::BACKGROUND_1_DAY;
		break;
	...
	case GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS:
		// New
	case GameMode::GAMEMODE_CHALLENGE_MINIBOSS_NIGHT:
	case GameMode::GAMEMODE_CHALLENGE_NIGHT:
	case GameMode::GAMEMODE_CHALLENGE_CUBELAND:
		mBackground = BackgroundType::BACKGROUND_2_NIGHT;
		break;
	...
	case GameMode::GAMEMODE_INTRO:
		// New
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_ZOMBOTANY:
	case GameMode::GAMEMODE_CHALLENGE_MINIBOSS_POOL:
	case GameMode::GAMEMODE_CHALLENGE_POOL:
	case GameMode::GAMEMODE_CHALLENGE_WHAT:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_CHAOS:
		mBackground = BackgroundType::BACKGROUND_3_POOL;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_STORMY_NIGHT:
		// New
	case GameMode::GAMEMODE_CHALLENGE_MINIBOSS_FOG:
	case GameMode::GAMEMODE_CHALLENGE_FOG:
		mBackground = BackgroundType::BACKGROUND_4_FOG;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_BUNGEE_BLITZ:
		// New
	case GameMode::GAMEMODE_CHALLENGE_MINIBOSS_ROOF:
	case GameMode::GAMEMODE_CHALLENGE_SHALLNOTPASS:
	case GameMode::GAMEMODE_CHALLENGE_ROOF:
		mBackground = BackgroundType::BACKGROUND_5_ROOF;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_FINAL_BOSS:
		// New
	case GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON:
	case GameMode::GAMEMODE_SURVIVAL_HARD_MOON:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON:
	case GameMode::GAMEMODE_CHALLENGE_MOON:
		mBackground = BackgroundType::BACKGROUND_6_BOSS;
		break;
	...
	case GameMode::GAMEMODE_SURVIVAL_NORMAL_DAY_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_DAY_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_DAY_FALL:
	case GameMode::GAMEMODE_CHALLENGE_ECLIPSE:
		mBackground = BackgroundType::BACKGROUND_DAY_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_NIGHT_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_NIGHT_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_NIGHT_FALL:
		mBackground = BackgroundType::BACKGROUND_NIGHT_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_POOL_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_POOL_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_POOL_FALL:
		mBackground = BackgroundType::BACKGROUND_POOL_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_FOG_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_FOG_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_FOG_FALL:
		mBackground = BackgroundType::BACKGROUND_FOG_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_ROOF_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_ROOF_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_ROOF_FALL:
		mBackground = BackgroundType::BACKGROUND_ROOF_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON_FALL:
	case GameMode::GAMEMODE_SURVIVAL_HARD_MOON_FALL:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON_FALL:
		mBackground = BackgroundType::BACKGROUND_MOON_FALL;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_DAY_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_DAY_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_DAY_WINTER:
	case GameMode::GAMEMODE_CHALLENGE_ICE:
		mBackground = BackgroundType::BACKGROUND_DAY_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_NIGHT_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_NIGHT_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_NIGHT_WINTER:
		mBackground = BackgroundType::BACKGROUND_NIGHT_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_POOL_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_POOL_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_POOL_WINTER:
		mBackground = BackgroundType::BACKGROUND_POOL_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_FOG_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_FOG_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_FOG_WINTER:
		mBackground = BackgroundType::BACKGROUND_FOG_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_ROOF_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_ROOF_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_ROOF_WINTER:
		mBackground = BackgroundType::BACKGROUND_ROOF_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_HARD_MOON_WINTER:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON_WINTER:
		mBackground = BackgroundType::BACKGROUND_MOON_WINTER;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_DAY_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_DAY_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_DAY_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_STREAM:
		mBackground = BackgroundType::BACKGROUND_DAY_SPRING;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_NIGHT_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_NIGHT_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_NIGHT_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_STREAM_NIGHT:
		mBackground = BackgroundType::BACKGROUND_NIGHT_SPRING;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_POOL_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_POOL_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_POOL_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_POOL_PARTY:
	case GameMode::GAMEMODE_CHALLENGE_FLOODED:
		mBackground = BackgroundType::BACKGROUND_POOL_SPRING;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_FOG_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_FOG_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_FOG_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_DARKNESS:
		mBackground = BackgroundType::BACKGROUND_FOG_SPRING;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_ROOF_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_ROOF_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_ROOF_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_BORING:
		mBackground = BackgroundType::BACKGROUND_ROOF_SPRING;
		break;

	case GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_HARD_MOON_SPRING:
	case GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON_SPRING:
	case GameMode::GAMEMODE_CHALLENGE_BORING2:
		mBackground = BackgroundType::BACKGROUND_MOON_SPRING;
		break;

	case GameMode::GAMEMODE_CHALLENGE_DEADPLANTS:
		mBackground = BackgroundType::BACKGROUND_DEADPLANTS;
		break;

	case GameMode::GAMEMODE_CHALLENGE_HEAVEN:
		mBackground = BackgroundType::BACKGROUND_HEAVEN;
		break;
	...
	if (mBackground == BackgroundType::BACKGROUND_1_DAY)
	{
		...
		if (mApp->IsAdventureMode() && mApp->IsFirstTimeAdventureMode())
		{
			...
			else if (mLevel >= DAY_LEVEL_INDEX + 2 && mLevel <= DAY_LEVEL_INDEX + 3)
			{
				mPlantRow[0] = PlantRowType::PLANTROW_DIRT;
				mPlantRow[4] = PlantRowType::PLANTROW_DIRT;
			}
	...
	else if (mBackground == BackgroundType::BACKGROUND_POOL_WINTER
		|| mBackground == BackgroundType::BACKGROUND_FOG_WINTER)
	{
		mPlantRow[0] = PlantRowType::PLANTROW_NORMAL;
		mPlantRow[1] = PlantRowType::PLANTROW_NORMAL;
		mPlantRow[2] = PlantRowType::PLANTROW_TILE;
		mPlantRow[3] = PlantRowType::PLANTROW_TILE;
		mPlantRow[4] = PlantRowType::PLANTROW_NORMAL;
		mPlantRow[5] = PlantRowType::PLANTROW_NORMAL;
	}
		...
		|| mBackground == BackgroundType::BACKGROUND_MOON_WINTER)
	{
		mPlantRow[0] = PlantRowType::PLANTROW_TILE;
		mPlantRow[1] = PlantRowType::PLANTROW_TILE;
		mPlantRow[2] = PlantRowType::PLANTROW_TILE;
		mPlantRow[3] = PlantRowType::PLANTROW_TILE;
		mPlantRow[4] = PlantRowType::PLANTROW_TILE;
		mPlantRow[5] = PlantRowType::PLANTROW_DIRT;
	}
	else if (mBackground == BackgroundType::BACKGROUND_DAY_SPRING 
		|| mBackground == BackgroundType::BACKGROUND_NIGHT_SPRING)
	{
		mPlantRow[0] = PlantRowType::PLANTROW_STREAM;
		mPlantRow[1] = PlantRowType::PLANTROW_STREAM;
		mPlantRow[2] = PlantRowType::PLANTROW_STREAM;
		mPlantRow[3] = PlantRowType::PLANTROW_STREAM;
		mPlantRow[4] = PlantRowType::PLANTROW_STREAM;
		mPlantRow[5] = PlantRowType::PLANTROW_DIRT;
	}
	else if (mBackground == BackgroundType::BACKGROUND_RESORT)
	{
	...
	else if (mBackground == BackgroundType::BACKGROUND_POOL_SPRING 
		|| mBackground == BackgroundType::BACKGROUND_FOG_SPRING)
	{
		mPlantRow[0] = PlantRowType::PLANTROW_POOL;
	...
	for (int x = 0; x < MAX_GRID_SIZE_X; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
		{
			...
			else if (mPlantRow[y] == PlantRowType::PLANTROW_TILE && x >= 0 && x <= 8)
			{
				mGridSquareType[x][y] = GridSquareType::GRIDSQUARE_TILE;
			}
			else if (mPlantRow[y] == PlantRowType::PLANTROW_STREAM && x >= 3 && x <= 5)
			{
				mGridSquareType[x][y] = GridSquareType::GRIDSQUARE_POOL;
			}
	...
	{
		CreateBushReanim(672 + LAWN_XMIN, -100 + LAWN_YMIN, MakeRenderOrder(RENDER_LAYER_BUSH, 0, 0), ReanimationType::REANIM_BUSHES3);
		CreateBushReanim(677 + LAWN_XMIN, 30 + LAWN_YMIN,	MakeRenderOrder(RENDER_LAYER_BUSH, 1, 0), ReanimationType::REANIM_BUSHES4);
		CreateBushReanim(685 + LAWN_XMIN, 120 + LAWN_YMIN,	MakeRenderOrder(RENDER_LAYER_BUSH, 2, 0), ReanimationType::REANIM_BUSHES5);
		CreateBushReanim(680 + LAWN_XMIN, 210 + LAWN_YMIN,	MakeRenderOrder(RENDER_LAYER_BUSH, 3, 0), ReanimationType::REANIM_BUSHES3);
		Reanimation* aBush5 = CreateBushReanim(675 + LAWN_XMIN, 300 + LAWN_YMIN, MakeRenderOrder(RENDER_LAYER_BUSH, 4, 0), ReanimationType::REANIM_BUSHES4);
	...
	if (StageHasGraveStones())
	{
		...
		else
		{
			...
			else if (mLevel >= NIGHT_LEVEL_INDEX + 1 && mLevel <= NIGHT_LEVEL_INDEX + 3)
			{
				AddGraveStones(6, 1, aLevelRNG);
				AddGraveStones(7, 1, aLevelRNG);
				AddGraveStones(8, 2, aLevelRNG);
			}
			else if (mLevel >= NIGHT_LEVEL_INDEX + 4 && mLevel <= NIGHT_LEVEL_INDEX + 6)
			{
				AddGraveStones(5, 1, aLevelRNG);
				AddGraveStones(6, 1, aLevelRNG);
				AddGraveStones(7, 2, aLevelRNG);
			...
			else if (mLevel >= NIGHT_LEVEL_INDEX + 7 && mLevel <= NIGHT_LEVEL_INDEX + 9)
			{
				AddGraveStones(4, 1, aLevelRNG);
			...
			else if (mLevel == NIGHT_LEVEL_INDEX + LEVELS_PER_AREA)
			{
...
bool Board::IsZombieWaveDistributionOk()
{
	...
	for (ZombieType aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType = (ZombieType)((int)aZombieType + 1))
	{
		if (aZombieType != ZombieType::ZOMBIE_YETI && aZombieType != ZombieType::ZOMBIE_BIGFOOT && CanZombieSpawnOnLevel(aZombieType, mLevel) && aZombieTypeCount[(int)aZombieType] == 0)
		{
...
void Board::InitZombieWaves()
{
	...
	mApp->mSawYeti = false;
	if (mApp->IsFirstTimeAdventureMode() && mLevel == DAY_LEVEL_INDEX + 2)
	{
...
{
	Rect aRect((mSeedBank->mWidth / 2) + BOARD_XCENTER, 0, Sexy::IMAGE_SHOVELBANK->GetWidth(), Sexy::IMAGE_SEEDBANK->GetHeight());
...
void Board::GetZenButtonRect(GameObjectType theObjectType, Rect& theRect)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		...
		{
			theRect.mX = BOARD_WIDTH - 270;
		}
		//return theRect;
	}

	theRect.mX = BOARD_XCENTER - 270;
	bool usable = true;
...
void Board::InitLevel()
{
	...
	else if (mApp->IsFirstTimeAdventureMode() && mLevel == DAY_LEVEL_INDEX + 1)
	{
		mSunMoney = 12;
	}
	else
	...
	if (StageHasSun())
	{
		...
		if (StageIsSunny())
			mSunCountDown *= SUNNY_MULTIPLIER;
		else if (StageHasEclipse())
			mSunCountDown *= ECLIPSE_MULTIPLIER;
	}
	// 初始化字幕播放记录
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_DAY)
	{
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_NIGHT)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_PUFFSHROOM);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_SUNSHROOM);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_FUMESHROOM);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_GRAVEBUSTER);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_HYPNOSHROOM);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_SCAREDYSHROOM);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_ICESHROOM);
		mSeedBank->mSeedPackets[7].SetPacketType(SeedType::SEED_DOOMSHROOM);
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_POOL)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 9);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_SUNFLOWER);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_LILYPAD);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_SQUASH);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_THREEPEATER);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_TANGLEKELP);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_JALAPENO);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_SPIKEWEED);
		mSeedBank->mSeedPackets[7].SetPacketType(SeedType::SEED_TORCHWOOD);
		mSeedBank->mSeedPackets[8].SetPacketType(SeedType::SEED_TALLNUT);
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_FOG)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 10); // 11
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_PUFFSHROOM);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_SUNSHROOM);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_LILYPAD);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_SEASHROOM);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_PLANTERN);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_CACTUS);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_BLOVER);
		mSeedBank->mSeedPackets[7].SetPacketType(SeedType::SEED_SPLITPEA);
		mSeedBank->mSeedPackets[8].SetPacketType(SeedType::SEED_STARFRUIT);
		mSeedBank->mSeedPackets[9].SetPacketType(SeedType::SEED_PUMPKINSHELL);
		//mSeedBank->mSeedPackets[10].SetPacketType(SeedType::SEED_MAGNETSHROOM);
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_ROOF)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 9);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_SUNFLOWER);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_CABBAGEPULT);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_FLOWERPOT);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_KERNELPULT);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_INSTANT_COFFEE);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_GARLIC);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_UMBRELLA);
		mSeedBank->mSeedPackets[7].SetPacketType(SeedType::SEED_MARIGOLD);
		mSeedBank->mSeedPackets[8].SetPacketType(SeedType::SEED_MELONPULT);
	}
	...
	if (StageHasFog())
	{
		...
		mFogOffset = 1025 + LAWN_XMIN - (LeftFogColumn() * 80);
	}
	// 关卡玩法相关的初始化
...
void Board::InitLawnMowers()
{
	...
	if (aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
		...
		mApp->IsLastStandLevel() || aGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM ||
	...
	for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
	{
		if ((aGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED && aRow <= 4) || 
			(mApp->IsAdventureMode() && mLevel == VASE_LEVEL) ||   // 这里原版没有对于行的判断，故冒险模式 4-5 关卡有 6 行小推车
...
bool Board::ChooseSeedsOnCurrentLevel()
{
	...
	if (
		//mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ICE || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_DAY || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_NIGHT || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_POOL || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_FOG || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ROOF || 
		//mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_MOON || 
...
void Board::UpdateLevelEndSequence()
{
	...
	{
		if (!IsSurvivalStageWithRepick() && !(mLevel % LEVELS_PER_AREA == LEVELS_PER_AREA - 1))
		{
...
void Board::FadeOutLevel()
{
	...
	if (!IsSurvivalStageWithRepick())
	{
		...
		if (mLevel % LEVELS_PER_AREA == LEVELS_PER_AREA - 1)
		{
...
bool Board::IsTileSquare(int theGridX, int theGridY)
{
	if (theGridX >= 0 && theGridY >= 0)
	{
		TOD_ASSERT(theGridX < MAX_GRID_SIZE_X && theGridY < MAX_GRID_SIZE_Y);
...
void Board::DoPlantingEffects(int theGridX, int theGridY, Plant* thePlant)
{
	...
	if (thePlant)
	{
		...
		else if (thePlant->mSeedType == SeedType::SEED_FLOWERPOT || thePlant->mSeedType == SeedType::SEED_WATERPOT || thePlant->mSeedType == SeedType::SEED_COFFEEPOT)
		{
			aYPos += 30;
		}
	}
	...
	if (mApp->GetConfigCheckbox(false, ConfigCheckbox::CONFIG_CHECKBOX_PLANT_SOUNDS)) {
		if (Plant::GetPlantSoundType(thePlant->mSeedType) == PlantSoundType::PLANT_SOUND_SUNFLOWER) mApp->PlayFoley(FoleyType::FOLEY_SUNFLOWER_PLANT);
		if (Plant::GetPlantSoundType(thePlant->mSeedType) == PlantSoundType::PLANT_SOUND_PEASHOOTER) mApp->PlayFoley(FoleyType::FOLEY_PEASHOOTER_PLANT);
	}
...
Plant* Board::GetCoffeePotAt(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mPlantCol == theGridX && aPlant->mRow == theGridY && !aPlant->NotOnGround() && aPlant->mSeedType == SeedType::SEED_COFFEEPOT)
		{
			return aPlant;
		}
	}
...
void Board::GetPlantsOnLawn(int theGridX, int theGridY, PlantsOnLawn* thePlantOnLawn)
{
	...
	while (IteratePlants(aPlant))
	{
		...
		if ((mApp->IsWallnutBowlingLevel() && Plant::IsWallNut(aSeedType)) || aSeedType == SeedType::SEED_BOWLNUT) continue;
		...
		else if (aSeedType == SeedType::SEED_FLOWERPOT || aSeedType == SeedType::SEED_WATERPOT || aSeedType == SeedType::SEED_COFFEEPOT || (aSeedType == SeedType::SEED_LILYPAD && IsPoolSquare(aPlant->mPlantCol, aPlant->mRow) && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN))
		{
...
int Board::CountSunFlowers()
{
	...
	while (IteratePlants(aPlant))
	{
		if (Plant::MakesSun(aPlant->mSeedType))
		{
			aCount++;
		}
	}
...
bool Board::CanZombieSpawnOnLevel(ZombieType theZombieType, int theLevel)
{
	...
	return gZombieAllowedLevels[theZombieType].mAllowedOnLevel[ClampInt(theLevel - 1, 0, FINAL_LEVEL - 1)];
}

//0x40D6F0
ZombieType Board::GetIntroducedZombieType()
{
	if (!mApp->IsAdventureMode() || mLevel == DAY_LEVEL_INDEX + 1)
	{
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex, ZombiePicker* theZombiePicker)
{
	...
	for (int aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType++)
	{
		...
		GameMode aGameMode = mApp->mGameMode;
		// 蹦极僵尸在无尽模式中仅在旗帜波出现
		if (Zombie::IsBungee(aZombieType) && mApp->IsSurvivalEndless(aGameMode))
		{
		...
		// 僵尸最早出现的波数的限制（出怪限制）
			...
			&& aGameMode != GameMode::GAMEMODE_CHALLENGE_ZOMBOWLING 
			&& aGameMode != GameMode::GAMEMODE_CHALLENGE_SHALLNOTPASS
			&& aGameMode != GameMode::GAMEMODE_CHALLENGE_POOL_PARTY
			)
		{
		...
		if (mApp->IsSurvivalMode())
		{
			int aFlags = GetSurvivalFlagsCompleted();
			// 伽刚特尔和雪橇车僵尸的每波出怪上限
			if (Zombie::IsGargantuar(aZombieType) && Zombie::IsGiga(aZombieType))
			{
				if (IsFlagWave(theWaveIndex))
				{
			...
			else if (Zombie::IsGargantuar(aZombieType) || Zombie::IsZomboni(aZombieType))
			{
...
bool Board::RowCanHaveZombieType(int theRow, ZombieType theZombieType)
{
	...
	if (!StageIsFlooded())
	{
		...
		{
			if (aCurrentWave < 5 && !Zombie::IsWaterOnly(theZombieType))
				return false;
			if (!Zombie::ZombieTypeCanGoInPool(theZombieType))
				return false;
		}
		else if (Zombie::IsWaterOnly(theZombieType))
		{
			return false;
		}
	}
	if (Zombie::IsBobsled(theZombieType) && !mIceTimer[theRow])
	{
		return false;
	}
	...
	//	}
	//}
	if (!Zombie::IsDancer(theZombieType) || StageHasPool())
	{
		return true;
	}
...
Zombie* Board::AddZombieInRow(ZombieType theZombieType, int theRow, int theFromWave)
{
	...
	// Giga Mode
	aZombie->ZombieInitialize(theRow, Zombie::ConvertZombieToGiga(theZombieType, mConfigCheckbox[CONFIG_CHECKBOX_GIGA_MODE]), aVariant, nullptr, theFromWave);
	if (Zombie::IsBobsled(theZombieType) && aZombie->IsOnBoard())
	{
		for (int _i = 0; _i < 3; _i++)
		{
			mZombies.DataArrayAlloc()->ZombieInitialize(theRow, Zombie::ConvertZombieToGiga(ZombieType::ZOMBIE_BOBSLED, mConfigCheckbox[CONFIG_CHECKBOX_GIGA_MODE]), false, aZombie, theFromWave);
		}
	}
	return aZombie;
}
...
Zombie* Board::AddZombie(ZombieType theZombieType, int theFromWave)
{
	...
	if (aRow >= 0 && aRow <= 4)
		RustleBush(aRow);
	return AddZombieInRow(theZombieType, aRow, theFromWave); 
}
...
PlantingReason Board::CanPlantAt(int theGridX, int theGridY, SeedType theSeedType)
{
	...
	if (theSeedType == SeedType::SEED_INSTANT_COFFEE)
	{
		...
		// Boost or awake check
		if (!aPlantOnLawn.mNormalPlant || aPlantOnLawn.mNormalPlant->mOnBungeeState == PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE || aPlantOnLawn.mNormalPlant->mWakeUpCounter > 0)
			return PlantingReason::PLANTING_NOT_HERE;
		if (!aPlantOnLawn.mNormalPlant->mIsAsleep)
		{
			if (!mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))
				return PlantingReason::PLANTING_NEEDS_SLEEPING;
			if (aPlantOnLawn.mNormalPlant->mIsBoosted)
				return PlantingReason::PLANTING_NEEDS_UNBOOSTED;
		}

		return PlantingReason::PLANTING_OK;
	}
	if (theSeedType == SeedType::SEED_HOTPOTATO)
	{
		if (aPlantOnLawn.mFlyingPlant)
			return PlantingReason::PLANTING_NOT_HERE;

		if (!aPlantOnLawn.mNormalPlant || aPlantOnLawn.mNormalPlant->mOnBungeeState == PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE)
			return PlantingReason::PLANTING_NOT_HERE;
		if (aPlantOnLawn.mNormalPlant->mIceTrapCounter > 0 || aPlantOnLawn.mNormalPlant->mChilledCounter > 0)
			return PlantingReason::PLANTING_OK;

		return PlantingReason::PLANTING_NEEDS_COLD;
	}
	if (aHasGrave)
	{
	...
	else
	{
		...
		aHasFlowerPot = aUnderPlant->mSeedType == SeedType::SEED_FLOWERPOT || aUnderPlant->mSeedType == SeedType::SEED_COFFEEPOT;
		aHasWaterPot = aUnderPlant->mSeedType == SeedType::SEED_WATERPOT;
	}
	...
	{
		if (!IsPoolSquare(theGridX, theGridY))
			return aHasWaterPot && !aNormalPlant && theSeedType != SeedType::SEED_LILYPAD ? PlantingReason::PLANTING_OK : PlantingReason::PLANTING_ONLY_IN_POOL;
	...
	{
		if (!aNormalPlant || theSeedType != SeedType::SEED_PUMPKINSHELL)
			return PlantingReason::PLANTING_NOT_ON_WATER;
	}
	if (Plant::IsRoof(theSeedType))
		return (aNormalPlant || aUnderPlant) ? PlantingReason::PLANTING_NOT_HERE : PlantingReason::PLANTING_OK;
	if ((StageHasRoof() || aGridSquare == GridSquareType::GRIDSQUARE_TILE) && !aHasFlowerPot)
	{
		if (!aNormalPlant || theSeedType != SeedType::SEED_PUMPKINSHELL)
	...
	if (aNormalPlant)
	{
		...
		// 坚果包扎术
		if (aNormalPlant->IsNut(theSeedType) && aAidPurchased)
		{
...
void Board::UpdateCursor()
{
	...
	case GameObjectType::OBJECT_TYPE_FAST_FORWARD_BUTTON:
		aShowFinger = true;
		break;
	...
	case GameObjectType::OBJECT_TYPE_PLANT:
		...
		if (((Plant*)aHitResult.mObject)->mState == PlantState::STATE_COBCANNON_READY || ((Plant*)aHitResult.mObject)->mPlantFoodReady)
		{
			aShowFinger = true;
		}
		break;
...
void Board::MouseDrag(int x, int y)
{
	...
	if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_COIN)
	{
		...
		if (aCoin->mBoard && !aCoin->IsLevelAward() && mApp->GetConfigCheckbox(false, CONFIG_CHECKBOX_HOLD_TO_COLLECT))
		{
...
void Board::UpdateMousePosition()
{
	...
	if (aCursorSeedType == SeedType::SEED_INSTANT_COFFEE)
	{
		...
		if (aPlant && (aPlant->mIsAsleep || (!aPlant->mIsBoosted && mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))) && CanPlantAt(aGridX, aGridY, SeedType::SEED_INSTANT_COFFEE) == PlantingReason::PLANTING_OK)
		{
			aPlant->mHighlighted = true;
		}
	}
	else if (aCursorSeedType == SeedType::SEED_HOTPOTATO)
	{
		Plant* aPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aPlant && (aPlant->mChilledCounter > 0 || aPlant->mChilledCounter > 0) && CanPlantAt(aGridX, aGridY, SeedType::SEED_HOTPOTATO) == PlantingReason::PLANTING_OK)
		{
			aPlant->mHighlighted = true;
		}
	}
	else if (Plant::IsNut(aCursorSeedType))
	{
	...
	{
		Plant* aPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aPlant && aPlant->mSeedType == aCursorSeedType && CanPlantAt(aGridX, aGridY, SeedType::SEED_PUMPKINSHELL) == PlantingReason::PLANTING_OK)
		{
			aPlant->mHighlighted = true;
		}
	}
...
void Board::UpdateToolTip()
{
	...
	if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO)
	{
		...
		if (mApp->CanShowAlmanac() && !Zombie::IsGargantuar(aZombie->mZombieType))
		{
		...
		if (Zombie::IsBungee(aZombie->mZombieType))
		{
	...
	if (gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST)
	{
		...
		else if (aUseSeedType == SeedType::SEED_FUTURESTAR)
		{
			mToolTip->SetLabel(_S("[BEGHOULED_FUTURESTAR_UPGRADE_TOOLTIP]"));
		}
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	if (aReason != PlantingReason::PLANTING_OK)
	{
		...
		{
			if (mApp->IsFirstTimeAdventureMode() && mLevel == ROOF_LEVEL_INDEX + 1)
			{
		...
		else if (aReason == PlantingReason::PLANTING_NEEDS_UNBOOSTED)
		{
			DisplayAdvice(_S("[ADVICE_PLANTING_NEED_UNBOOSTED]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANTING_NEED_SLEEPING);
		}
		else if (aReason == PlantingReason::PLANTING_NEEDS_COLD)
		{
			DisplayAdvice(_S("[ADVICE_PLANTING_NEED_COLD]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANTING_NEED_SLEEPING);
		}
	...
	if (aNormalPlant->IsNut(aPlantingSeedType) && aNormalPlant)
	{
	...
	if (mApp->IsColumnLevel())
	{
		for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
		{
			...
			if (aNormalPlant->IsNut(aPlantingSeedType))
			{
	...
	// 保龄球关卡，播放保龄球滚动的音效
	if ((mApp->IsWallnutBowlingLevel() && Plant::IsWallNut(aPlantingSeedType)) || aPlantingSeedType == SeedType::SEED_BOWLNUT)
	{
...
bool Board::MouseHitTestPlant(int x, int y, HitResult* theHitResult)
{
	...
	else
	{
		...
		if (aPlant && (aPlant->mSeedType == SeedType::SEED_LILYPAD || aPlant->mSeedType == SeedType::SEED_FLOWERPOT || aPlant->mSeedType == SeedType::SEED_WATERPOT || aPlant->mSeedType == SeedType::SEED_COFFEEPOT))
		{
...
bool Board::MouseHitTest(int x, int y, HitResult* theHitResult)
{
	...
	// Fast Forward
	if (mFastForwardButton->IsMouseOver() && CanInteractWithBoardButtons())
	{
		theHitResult->mObjectType = GameObjectType::OBJECT_TYPE_FAST_FORWARD_BUTTON;
		return true;
	}
	...
	if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM) && CanInteractWithBoardButtons())
	{
		for (int i = (int)GameObjectType::OBJECT_TYPE_WATERING_CAN; i <= (int)GameObjectType::OBJECT_TYPE_NEXT_GARDEN; i++)
		{
			...
			if (CanUseGameObject(aTool) && (aTool != GameObjectType::OBJECT_TYPE_TREE_FOOD || mChallenge->TreeOfWisdomCanFeed()))
			{
				...
				{
					aButtonRect.mX = BOARD_WIDTH - 236;
				}
				else
				{
...
void Board::MouseDown(int x, int y, int theClickCount)
{
	...
	if ((mMenuButton->IsMouseOver() || mFastForwardButton->IsMouseOver()) && CanInteractWithBoardButtons() && theClickCount > 0)
	{
	...
	if (aZombie != nullptr && CanTakeShadowSunMoney(4)) {
		aZombie->TakeDamage(900, 0U);
		TakeShadowSunMoney(4);
	}
...
void Board::MouseUp(int x, int y, int theClickCount)
{
	...
	if (CanInteractWithBoardButtons() && theClickCount > 0)
	{
		...
		// Fast Forward
		else if (mFastForwardButton->IsMouseOver() && !mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) && !mApp->GetDialog(Dialogs::DIALOG_LEVEL_COMPLETE))
		{
			int aFastForwardSound = mFastForward ? Sexy::SOUND_FLOOP : Sexy::SOUND_WAKEUP;
			mApp->PlaySample(aFastForwardSound);
			mFastForward = !mFastForward;
		}
...
void Board::SpawnZombiesFromPool()
{
	...
	if ((mLevel >= POOL_LEVEL_INDEX + 1 && mLevel <= POOL_LEVEL_INDEX + 2) || (mLevel >= FOG_LEVEL_INDEX + 1 && mLevel <= FOG_LEVEL_INDEX + 2))
	{
		aCount = 2;
		aZombiePoints = 3;
	}
	else if ((mLevel >= POOL_LEVEL_INDEX + 3 && mLevel <= POOL_LEVEL_INDEX + 5) || (mLevel >= FOG_LEVEL_INDEX + 3 && mLevel <= FOG_LEVEL_INDEX + 5))
	{
		aCount = 3;
		aZombiePoints = 5;
	}
	...
	for (int aGridX = aColStart; aGridX <= aColEnd; aGridX++)
	{
		for (int aGridY = aRowStart; aGridY <= aRowEnd; aGridY++)
		{
...
void Board::SpawnZombiesFromSky()
{
	...
	int aCount, aZombiePoints;
	if ((mLevel >= ROOF_LEVEL_INDEX + 1 && mLevel <= ROOF_LEVEL_INDEX + 2) || (mLevel >= MOON_LEVEL_INDEX + 1 && mLevel <= MOON_LEVEL_INDEX + 2))
	{
		aCount = 2;
		aZombiePoints = 3;
	}
	else if ((mLevel >= ROOF_LEVEL_INDEX + 3 && mLevel <= ROOF_LEVEL_INDEX + 5) || (mLevel >= MOON_LEVEL_INDEX + 3 && mLevel <= MOON_LEVEL_INDEX + 5))
	{
		aCount = 3;
		aZombiePoints = 5;
	}
...
{
	if (mApp->IsZombotanyLevel() || mApp->IsMinecraftLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SUNNY_DAY)
		return;
	...
	else if (StageHasWater() && !StageHasGraveStones())
	{
	...
	while (IterateGridItems(aGridItem))
	{
		...
		if (StageHasGraveDanger() && Rand(mNumWaves) > mCurrentWave)
		{
			continue;
		}
...
int Board::TotalZombiesHealthInWave(int theWaveIndex)
{
	...
	while (IterateZombies(aZombie))
	{
		if (aZombie->mFromWave == theWaveIndex && !aZombie->mMindControlled && !aZombie->IsDeadOrDying() &&
			!Zombie::IsBungee(aZombie->mZombieType) && aZombie->mRelatedZombieID == ZombieID::ZOMBIEID_NULL)
		{
			aTotalHealth += aZombie->mBodyHealth + aZombie->mHelmHealth + aZombie->mShieldHealth * 0.2f + aZombie->mArmorHealth * 0.4f + aZombie->mFlyingHealth;
		}
	}
	return aTotalHealth;
}
...
void Board::SpawnZombieWave()
{
	...
	if (mApp->IsBungeeBlitzLevel())
	{
		...
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
			...
			if (Zombie::IsBungee(aZombieType) || Zombie::IsZomboni(aZombieType))
			{
	...
	else
	{
		...
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
			...
			if (Zombie::IsBobsled(aZombieType) && !CanAddBobSled())
			{
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	while (IterateZombies(aZombie))
	{
		...
		{
			if (Zombie::IsGargantuar(aZombie->mZombieType) && aZombie->IsDeadOrDying() && aZombie->mPosX < 140)
			{
				aZombie->DieNoLoot();
			}
		}
	...
	Reanimation* aReanim = mApp->AddReanimation(-BOARD_OFFSET_X, -BOARD_OFFSET_Y, MakeRenderOrder(RenderLayer::RENDER_LAYER_SCREEN_FADE, 0, 0), ReanimationType::REANIM_ZOMBIES_WON);
	aReanim->mLoopType = ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD;
...
const bool Board::IsLastStandFinalStage()
{
...
void Board::UpdateSunSpawning()
{
	...
	if (StageHasEclipse())
		mSunCountDown *= ECLIPSE_MULTIPLIER;
	CoinType aSunType = mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SUNNY_DAY ? CoinType::COIN_LARGESUN : StageHasEclipse() ? CoinType::COIN_SMALLSUN :CoinType::COIN_SUN;
...
void Board::NextWaveComing()
{
	...
	{
		if (!IsSurvivalStageWithRepick() && !mApp->IsLastStandLevel() && !mApp->IsContinuousChallenge())
		{
	...
	if (mCurrentWave == 0)
	{
		mApp->IsDeadplantsLevel() ? mApp->PlaySample(Sexy::SOUND_DEADPLANTS) : mApp->PlaySample(Sexy::SOUND_AWOOGA);
	}
...
void Board::UpdateZombieSpawning()
{
	...
	if (mZombieCountDown == 5)
	{
		...
		{
			ClearAdviceImmediately();
			mApp->IsDeadplantsLevel() ? 
				DisplayAdviceAgain(_S("[ADVICE_HUGE_WAVE_DEADPLANTS]"), MessageStyle::MESSAGE_STYLE_HUGE_WAVE, AdviceType::ADVICE_HUGE_WAVE) : 
	...
	if (mZombieCountDown == 0)
	{
		...
		else
		{
			...
			if (mApp->IsLittleTroubleLevel() || mApp->IsColumnLevel() || mApp->IsLastStandLevel())
			{
...
void Board::UpdateTutorial()
{
	...
	if (mApp->IsFirstTimeAdventureMode() && mLevel >= DAY_LEVEL_INDEX + 3 && mLevel != BOWLING_LEVEL && mLevel <= DAY_LEVEL_INDEX + 7 && mTutorialState == TutorialState::TUTORIAL_OFF &&
...
void Board::UpdateGame()
{
	...
	{
		float aMaxFogOffset = 1025.0f + LAWN_XMIN - (LeftFogColumn() * 80.0f);
		if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO)
		{
			mFogOffset = TodAnimateCurveFloat(200, 0, mFogBlownCountDown, aMaxFogOffset, -40 + LAWN_XMIN, TodCurves::CURVE_EASE_OUT);
		}
		else if (mFogBlownCountDown < 2000)
		{
			mFogOffset = TodAnimateCurveFloat(2000, 0, mFogBlownCountDown, aMaxFogOffset, -40 + LAWN_XMIN, TodCurves::CURVE_EASE_OUT);
		}
		else if (mFogOffset < aMaxFogOffset)
		{
			mFogOffset = TodAnimateCurveFloat(-5, aMaxFogOffset, mFogOffset * 1.1f, -40 + LAWN_XMIN, aMaxFogOffset, TodCurves::CURVE_LINEAR);
		}
	}
	...
	if (mMainCounter == 1 && mApp->IsFirstTimeAdventureMode())
	{
		...
		else if (mLevel == DAY_LEVEL_INDEX + 2)
		{
			SetTutorialState(TutorialState::TUTORIAL_LEVEL_2_PICK_UP_SUNFLOWER);
...
void Board::Update()
{
	...
	mMenuButton->Update();
	// Fast Forward
	if (!mFastForwardButton->mBtnNoDraw)
	{
		mFastForwardButton->mDisabled = aDisabled;
	}
	mFastForwardButton->Update();
	mFastForwardButton->mButtonImage = mFastForward ? Sexy::IMAGE_BUTTON_FASTFORWARD_DOWN : Sexy::IMAGE_BUTTON_FASTFORWARD;
	if (mStoreButton)
	{
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	{
	case BackgroundType::BACKGROUND_1_DAY:
	case BackgroundType::BACKGROUND_DAY_SPRING:			aBgImage = Sexy::IMAGE_BACKGROUND1;						break;
	case BackgroundType::BACKGROUND_DAY_FALL:			aBgImage = Sexy::IMAGE_BACKGROUNDE;						break;
	case BackgroundType::BACKGROUND_DAY_WINTER:			aBgImage = Sexy::IMAGE_BACKGROUND1W;					break;
	case BackgroundType::BACKGROUND_2_NIGHT:
	case BackgroundType::BACKGROUND_NIGHT_FALL:
	case BackgroundType::BACKGROUND_NIGHT_WINTER:
	case BackgroundType::BACKGROUND_NIGHT_SPRING:		aBgImage = Sexy::IMAGE_BACKGROUND2;						break;
	case BackgroundType::BACKGROUND_3_POOL:
	case BackgroundType::BACKGROUND_POOL_FALL:
	case BackgroundType::BACKGROUND_POOL_WINTER:
	case BackgroundType::BACKGROUND_POOL_SPRING:		aBgImage = Sexy::IMAGE_BACKGROUND3;						break;
	case BackgroundType::BACKGROUND_4_FOG:
	case BackgroundType::BACKGROUND_FOG_FALL:
	case BackgroundType::BACKGROUND_FOG_WINTER:
	case BackgroundType::BACKGROUND_FOG_SPRING:			aBgImage = Sexy::IMAGE_BACKGROUND4;						break;
	case BackgroundType::BACKGROUND_5_ROOF:
	case BackgroundType::BACKGROUND_ROOF_FALL:
	case BackgroundType::BACKGROUND_ROOF_WINTER:
	case BackgroundType::BACKGROUND_ROOF_SPRING:		aBgImage = Sexy::IMAGE_BACKGROUND5;						break;
	case BackgroundType::BACKGROUND_6_BOSS:
	case BackgroundType::BACKGROUND_MOON_FALL:
	case BackgroundType::BACKGROUND_MOON_WINTER:
	case BackgroundType::BACKGROUND_MOON_SPRING:		aBgImage = Sexy::IMAGE_BACKGROUND6BOSS;					break;
	case BackgroundType::BACKGROUND_RESORT:				aBgImage = Sexy::IMAGE_BACKGROUNDR;						break;
	...
	case BackgroundType::BACKGROUND_DEADPLANTS:			aBgImage = Sexy::IMAGE_BACKGROUNDD;						break;
	case BackgroundType::BACKGROUND_HEAVEN:				aBgImage = Sexy::IMAGE_BACKGROUNDH;						break;
	default:											TOD_ASSERT();											break;
	}

	if (mLevel == DAY_LEVEL_INDEX + 1 && mApp->IsFirstTimeAdventureMode())
	{
	...
	else if (((mLevel >= DAY_LEVEL_INDEX + 2 && mLevel <= DAY_LEVEL_INDEX + 3) && mApp->IsFirstTimeAdventureMode()) || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED)
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET_X, -BOARD_OFFSET_Y);
		g->DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET_X, 265 - BOARD_OFFSET_Y);
		...
		g->DrawImage(Sexy::IMAGE_SOD3ROW, 235 - BOARD_OFFSET_X, 149 - BOARD_OFFSET_Y, aSrcRect);
	}
	else if (mLevel == DAY_LEVEL_INDEX + 4 && mApp->IsFirstTimeAdventureMode())
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET_X, -BOARD_OFFSET_Y);
		g->DrawImage(Sexy::IMAGE_SOD3ROW, 235 - BOARD_OFFSET_X, 149 - BOARD_OFFSET_Y);
		...
		g->DrawImage(Sexy::IMAGE_BACKGROUND1, 232 - BOARD_OFFSET_X, -BOARD_OFFSET_Y, aSrcRect);
	}
	else if (mApp->IsMinecraftLevel())
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUNDM, -BOARD_OFFSET_X, -BOARD_OFFSET_Y);
	}
	else if (aBgImage)
	{
		...
		{
			g->DrawImage(aBgImage, -BOARD_OFFSET_X, -BOARD_OFFSET_Y);
		}
	}
	...
	if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PLANT_PEASHOOTER)
	{
		...
		aClipG.DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET_X, 265 - BOARD_OFFSET_Y);
...
void Board::DrawProgressMeter(Graphics* g)
{
	...
	// ▲ 绘制进度条进度部分的贴图
	// ====================================================================================================
	g->DrawImageCel(Sexy::IMAGE_FLAGMETER, BOARD_WIDTH - 200, BOARD_HEIGHT - 25, 0);
	...
	Rect aDstRect(aCelWidth - aClipWidth + BOARD_WIDTH - 207, BOARD_HEIGHT - 25, aClipWidth, aCelHeight);
	g->DrawImage(Sexy::IMAGE_FLAGMETER, aDstRect, aSrcRect);
	
	// ====================================================================================================
	// ▲ 根据不同关卡，绘制进度条上的文字或旗帜
	// ====================================================================================================
	int aPosX = aCelWidth / 2 + BOARD_WIDTH - 200;
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST)
	{
		...
		TodDrawString(g, aMatchStr, aPosX, BOARD_HEIGHT - 11, Sexy::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	...
	{
		int aSunMoney = ClampInt(mSunMoney, 0, 160);
		SexyString aMatchStr = StrFormat(_S("%d / %d %s"), aSunMoney, 160, TodStringTranslate(_S("[SUN]")).c_str());
		TodDrawString(g, aMatchStr, aPosX, BOARD_HEIGHT - 11, Sexy::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM)
	{
		int aSunMoney = ClampInt(mSunMoney, 0, 80);
		SexyString aMatchStr = StrFormat(_S("%d / %d %s"), aSunMoney, 80, TodStringTranslate(_S("[SUN]")).c_str());
		TodDrawString(g, aMatchStr, aPosX, BOARD_HEIGHT - 11, Sexy::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->IsIZombieLevel())
	{
	...
	else if (ProgressMeterHasFlags())
	{
		...
		int aFlagsPosEnd = BOARD_WIDTH - 210 + aCelWidth;  // 旗帜区域的右界横坐标
		for (int aFlagWave = 1; aFlagWave <= aNumFlagWaves; aFlagWave++)
		{
			...
			// 计算旗帜的横坐标
			int aPosX = TodAnimateCurve(0, mNumWaves, aTotalWavesAtFlag, aFlagsPosEnd, BOARD_WIDTH - 194, TodCurves::CURVE_LINEAR);
			// 绘制旗杆
			g->DrawImageCel(Sexy::IMAGE_FLAGMETERPARTS, aPosX, BOARD_HEIGHT - 29, 1, 0);
			// 绘制旗帜
			g->DrawImageCel(Sexy::IMAGE_FLAGMETERPARTS, aPosX, BOARD_HEIGHT - 28 - aHeight, 2, 0);
		}
	}
	...
	// ====================================================================================================
	// 绘制“关卡进程”的小牌子
	g->DrawImage(Sexy::IMAGE_FLAGMETERLEVELPROGRESS, BOARD_WIDTH - 162, BOARD_HEIGHT - 11);
	...
	g->DrawImageCel(Sexy::IMAGE_FLAGMETERPARTS, aCelWidth - aHeadProgress + BOARD_WIDTH - 220, BOARD_HEIGHT - 28, 0, 0);
}
...
void Board::DrawHouseDoorBottom(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_DAY_SPRING:
	case BackgroundType::BACKGROUND_RESORT:
	case BackgroundType::BACKGROUND_HEAVEN:			g->DrawImage(Sexy::IMAGE_BACKGROUND1_GAMEOVER_INTERIOR_OVERLAY, -126, 225);		break;
	case BackgroundType::BACKGROUND_DAY_FALL:		g->DrawImage(Sexy::IMAGE_BACKGROUNDE_GAMEOVER_INTERIOR_OVERLAY, -125, 196);		break;
	case BackgroundType::BACKGROUND_2_NIGHT:
	case BackgroundType::BACKGROUND_NIGHT_FALL:
	case BackgroundType::BACKGROUND_NIGHT_WINTER:
	case BackgroundType::BACKGROUND_NIGHT_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND2_GAMEOVER_INTERIOR_OVERLAY, -125, 196);		break;
	case BackgroundType::BACKGROUND_3_POOL:
	case BackgroundType::BACKGROUND_POOL_FALL:
	case BackgroundType::BACKGROUND_POOL_WINTER:
	case BackgroundType::BACKGROUND_POOL_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND3_GAMEOVER_INTERIOR_OVERLAY, -171, 241);		break;
	case BackgroundType::BACKGROUND_4_FOG:
	case BackgroundType::BACKGROUND_FOG_FALL:
	case BackgroundType::BACKGROUND_FOG_WINTER:
	case BackgroundType::BACKGROUND_FOG_SPRING:		g->DrawImage(Sexy::IMAGE_BACKGROUND4_GAMEOVER_INTERIOR_OVERLAY, -172, 246);		break;
	case BackgroundType::BACKGROUND_DEADPLANTS:		g->DrawImage(Sexy::IMAGE_BACKGROUNDD_GAMEOVER_INTERIOR_OVERLAY, -125, 196);		break;
	default:																													break;
	}
}
...
void Board::DrawHouseDoorTop(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_DAY_SPRING:
	case BackgroundType::BACKGROUND_RESORT:
	case BackgroundType::BACKGROUND_HEAVEN:			g->DrawImage(Sexy::IMAGE_BACKGROUND1_GAMEOVER_MASK, -130, 202);		break;
	case BackgroundType::BACKGROUND_DAY_FALL:		g->DrawImage(Sexy::IMAGE_BACKGROUNDE_GAMEOVER_MASK, -128, 207);		break;
	case BackgroundType::BACKGROUND_2_NIGHT:
	case BackgroundType::BACKGROUND_NIGHT_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND2_GAMEOVER_MASK, -128, 207);		break;
	case BackgroundType::BACKGROUND_3_POOL:
	case BackgroundType::BACKGROUND_POOL_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND3_GAMEOVER_MASK, -172, 234);		break;
	case BackgroundType::BACKGROUND_4_FOG:
	case BackgroundType::BACKGROUND_FOG_SPRING:		g->DrawImage(Sexy::IMAGE_BACKGROUND4_GAMEOVER_MASK, -173, 133);		break;
	case BackgroundType::BACKGROUND_5_ROOF:
	case BackgroundType::BACKGROUND_ROOF_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND5_GAMEOVER_MASK, -220, 81);		break;
	case BackgroundType::BACKGROUND_6_BOSS:
	case BackgroundType::BACKGROUND_MOON_SPRING:	g->DrawImage(Sexy::IMAGE_BACKGROUND6_GAMEOVER_MASK, -220, 81);		break;
	case BackgroundType::BACKGROUND_DEADPLANTS:		g->DrawImage(Sexy::IMAGE_BACKGROUNDD_GAMEOVER_MASK, -128, 207);		break;
	default:																										break;
	}
}
...
void Board::DrawTopRightUI(Graphics* g)
{
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		...
		{
			mMenuButton->mY = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, MENU_BUTTON_Y, -50, TodCurves::CURVE_EASE_IN_OUT);
			mStoreButton->mX = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 678, 800, TodCurves::CURVE_EASE_IN_OUT);
		}
		else
		{
			mMenuButton->mY = MENU_BUTTON_Y;
			mStoreButton->mX = 678;
		}
	}
...
void Board::ClearFogAroundPlant(Plant* thePlant, int theSize)
{
	...
	int aFogOffsetX = (mFogOffset + 90 - LAWN_XMIN) / 100;
...
void Board::UpdateFog()
{
	...
	while (IteratePlants(aPlant))
	{
		...
		if (aPlant->mSeedType == SeedType::SEED_PLASMAPEA || aPlant->mSeedType == SeedType::SEED_FLAMEWOOD)
		{
			ClearFogAroundPlant(aPlant, 2);
		}
		else if (aPlant->mSeedType == SeedType::SEED_TORCHWOOD || aPlant->mSeedType == SeedType::SEED_FIREPEA)
		{
...
void Board::DrawFog(Graphics* g)
{
	...
	for (int x = 0; x < MAX_GRID_SIZE_X; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y + 1; y++)
		{
			...
			float aPosY = (y * 85) - 60 + LAWN_YMIN; // Fog Fix Maybe
...
// U53RDV
void Board::SetDevMode(bool theEnableFuture)
{
	mApp->PlaySample(Sexy::SOUND_BOING);
	mDevMode = theEnableFuture;
	mApp->mDevMode = theEnableFuture;

	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
		aZombie->EnableDevMode(theEnableFuture);
	}
}
...
void Board::DoTypingCheck(KeyCode theKey)
{
	...
	if (mApp->mDevModeCheck->Check(theKey))
	{
		SetDevMode(!mDevMode);
		return;
	}
...
void Board::KeyDown(KeyCode theKey)
{
	...
	// Slot Hotkeys
	if (mSeedBank != nullptr && gLawnApp->GetConfigCheckbox(false, CONFIG_CHECKBOX_SLOT_HOTKEYS)) {
		KeyDownSeedbank(theKey);
	}
}

void Board::KeyDownSeedbank(KeyCode theKey)
 {
	if (theKey == _S('1') || theKey == _S('2') || theKey == _S('3') || theKey == _S('4') || theKey == _S('5') || theKey == _S('6') || theKey == _S('7') || theKey == _S('8') || theKey == _S('9') || theKey == _S('0')) {
	...
	case _S('1'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[0];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('2'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[1];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('3'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[2];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('4'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[3];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('5'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[4];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('6'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[5];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('7'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[6];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('8'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[7];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('9'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[8];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
	case _S('0'):
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[9];
		aPacket->MouseDown(0, 0, 0);
		break;
	}
...
void Board::KeyChar(SexyChar theChar)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		...
		if (theChar == _S(']'))
		{
			...
			if (aPottedPlant)
			{
				...
				if (aPottedPlant->mSeedType == SeedType::SEED_WATERPOT)
				{
					aPottedPlant->mSeedType = SeedType::SEED_ICEBERGLETTUCEBETA;
				}
			}
			return;
		}
	...
	if (theChar == _S('<'))
	{
		...
		else
		{
			...
			for (int y = 0; y < MAX_GRID_SIZE_Y; ++y)
			{
				for (int x = 0; x < MAX_GRID_SIZE_X; ++x)
				{
					if (StageHasTiles() && IsTileSquare(x, y) && CanPlantAt(x, y, SeedType::SEED_FLOWERPOT) == PlantingReason::PLANTING_OK)
					{
	...
	if (theChar == _S('q'))
	{
		AddZombie(ZombieType::ZOMBIE_NORMAL, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('Q'))
	{
		AddZombie(ZombieType::ZOMBIE_MUSTACHE, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('w'))
	{
	...
	if (theChar == _S('W'))
	{
		AddZombie(ZombieType::ZOMBIE_RALLY, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('e'))
	{
	...
	if (theChar == _S('E'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGACONE, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('r'))
	{
	...
	if (theChar == _S('R'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAVAULTER, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('t'))
	{
	...
	if (theChar == _S('T'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGABUCKET, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('y'))
	{
	...
	if (theChar == _S('Y'))
	{
		AddZombie(ZombieType::ZOMBIE_SUNDAY, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('u'))
	{
	...
	if (theChar == _S('U'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGADOOR, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('i'))
	{
	...
	if (theChar == _S('I'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAFOOTBALL, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('o'))
	{
	...
	{
		if (StageHasWater())
			AddZombie(ZombieType::ZOMBIE_GIGASNORKEL, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('['))
	{
	...
	if (theChar == _S('{'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAZOMBONI, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S(']'))
	{
	...
	if (theChar == _S('}'))
	{
		...
		AddZombie(ZombieType::ZOMBIE_GIGABOBSLED, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('a'))
	{
	...
	{
		if (StageHasWater())
			AddZombie(ZombieType::ZOMBIE_ORCA_RIDER, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('s'))
	{
	...
	if (theChar == _S('S'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAJACK, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('d'))
	{
	...
	if (theChar == _S('D'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGABALLOON, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('f'))
	{
	...
	if (theChar == _S('G'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGADIGGER, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('h'))
	{
	...
	if (theChar == _S('H'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAPOGO, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('j'))
	{
	...
	if (theChar == _S('J'))
	{
		AddZombie(ZombieType::ZOMBIE_BIGFOOT, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('k'))
	{
	...
	if (theChar == _S('K'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGABUNGEE, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('l'))
	{
	...
	if (theChar == _S('L'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGALADDER, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S(';'))
	{
	...
	if (theChar == _S(':'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGACATAPULT, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('z'))
	{
	...
	if (theChar == _S('Z'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAGARGANTUAR, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('x'))
	{
		AddZombie(ZombieType::ZOMBIE_IMP, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('X'))
	{
		AddZombie(ZombieType::ZOMBIE_GIGAIMP, Zombie::ZOMBIE_WAVE_DEBUG);
		return;
	}
	if (theChar == _S('c'))
	{
	...
	if (theChar == _S('*') && mApp->mCtrlDown && mApp->mTodCheatKeys)
	{
		TodCrash();
...
void Board::AddShadowSunMoney(int theAmount)
{
	mShadowSunMoney += theAmount;
	if (mShadowSunMoney > 9999)
	{
		mShadowSunMoney = 9999;
	}
}

int Board::CountShadowSunBeingCollected()
{
	int aCount = 0;
	Coin* aCoin = nullptr;
	while (IterateCoins(aCoin))
	{
		if (aCoin->mIsBeingCollected && aCoin->mType == COIN_SHADOW_SUN)
		{
			aCount += aCoin->mValue;
		}
	}
...
bool Board::TakeShadowSunMoney(int theAmount)
{
	if (CanTakeShadowSunMoney(theAmount))
	{
		mShadowSunMoney -= theAmount;
		return true;
	}
...
bool Board::CanTakeShadowSunMoney(int theAmount)
{
	return theAmount <= mShadowSunMoney + CountShadowSunBeingCollected();
}
...
int Board::GetNumSeedsInBank()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_8)
		return 6;
	if (mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_9 || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_DAY || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_NIGHT)
		return 8;
	if (mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_POOL || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ROOF)
		return 9;
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_FOG)
		return 10;
...
{
	if (StageHasStorm() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL)
		return false;
	return mBackground == BackgroundType::BACKGROUND_4_FOG || mBackground == BackgroundType::BACKGROUND_FOG_FALL || mBackground == BackgroundType::BACKGROUND_FOG_WINTER || mBackground == BackgroundType::BACKGROUND_DEADPLANTS;
}
...
int Board::LeftFogColumn()
{
	...
	if (mLevel == FOG_LEVEL_INDEX + 1)													return 6;
	if (mLevel >= FOG_LEVEL_INDEX + 2 && mLevel <= FOG_LEVEL_INDEX + 6)					return 5;
	if (mLevel >= FOG_LEVEL_INDEX + 7 && mLevel <= FOG_LEVEL_INDEX + LEVELS_PER_AREA)	return 4;
																						return 5;
}
...
int Board::GetSeedPacketPositionX(int theIndex)
{
	...
	if (mSeedBank->mNumPackets == 8)		return theIndex * 54 + 81;
	if (mSeedBank->mNumPackets == 9)		return theIndex * 52 + 80;
...
int Board::PlantingPixelToGridY(int theX, int theY, SeedType theSeedType)
{
	...
	{
		Plant* aPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aPlant && (aPlant->mIsAsleep || (!aPlant->mIsBoosted && mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))))
		{
			return aGridY;
		}
		...
		if (aGridYDown != aGridY)
		{
			...
			if (aPlantDown && aPlant && (aPlant->mIsAsleep || (!aPlant->mIsBoosted && mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))))
			{
				return aGridYDown;
			}
		}
		...
		if (aGridYUp != aGridY)
		{
			...
			if (aPlantUp && aPlant && (aPlant->mIsAsleep || (!aPlant->mIsBoosted && mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))))
			{
				return aGridYUp;
			}
		}
	}
	else if (theSeedType == SeedType::SEED_HOTPOTATO)
	{
		Plant* aPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aPlant && (aPlant->mIceTrapCounter > 0 || aPlant->mChilledCounter > 0))
		{
			return aGridY;
		}
		...
		{
			Plant* aPlantDown = GetTopPlantAt(aGridX, aGridYDown, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
			if (aPlantDown && aPlant && (aPlant->mIceTrapCounter > 0 || aPlant->mChilledCounter > 0))
			{
				return aGridYDown;
			}
		}
		...
		{
			Plant* aPlantUp = GetTopPlantAt(aGridX, aGridYUp, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
			if (aPlantUp && aPlant && (aPlant->mIceTrapCounter > 0 || aPlant->mChilledCounter > 0))
			{
				return aGridYUp;
			}
		}
...
float Board::GetPosYBasedOnRow(float thePosX, int theRow)
{
	if (StageHasRoof())
	{
		...
		if (thePosX < 400.0f + LAWN_XMIN)
		{
			aSlopeOffset = (400.0f + LAWN_XMIN - thePosX) * 0.25f;
		}
...
//0x41CBF0
void Board::KillAllPlantsInRadius(int theX, int theY, int theRadius, int theDamage)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		...
		{
			Plant* aNormalPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
			Plant* aPumpkinPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_PUMPKIN);

			if (aNormalPlant != nullptr && aNormalPlant->mPlantHealth > 1800 && aNormalPlant != aPlant && aPumpkinPlant != aPlant) continue;
			if (aPumpkinPlant != nullptr && aPumpkinPlant->mPlantHealth > 1800 && aPumpkinPlant != aPlant) continue;

			if (aPlant->mPlantHealth > theDamage) {
...
unsigned int Board::SeedNotRecommendedForLevel(SeedType theSeedType)
{
	...
	if (theSeedType == SeedType::SEED_INSTANT_COFFEE && StageIsNight() && !mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_COFFEE_BOOST))
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_AT_NIGHT, true);
	}
	if (theSeedType == SeedType::SEED_HOTPOTATO && !StageIsWinter() && !(mApp->IsZombotanyLevel() && mConfigCheckbox[CONFIG_CHECKBOX_GIGA_MODE]))
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_COLD, true);
	}
	...
	if (theSeedType == SeedType::SEED_FLOWERPOT && !StageHasTiles())
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_ROOF, true);
	}
	if (!StageHasWater() && Plant::IsAquatic(theSeedType))
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_POOL, true);
	}
	if ((theSeedType == SeedType::SEED_POTATOMINE || theSeedType == SeedType::SEED_SPIKEWEED || theSeedType == SeedType::SEED_SPIKEROCK || theSeedType == SeedType::SEED_WATERPOT) && StageIsFlooded())
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_SOLID_GROUND, true);
	}
	return aNotRec;
}
...
void Board::DropLootPiece(int thePosX, int thePosY, int theDropFactor)
{
	...
	{
		if (mLevel == POOL_LEVEL_INDEX + 2 && mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedMinigames && CountCoinByType(CoinType::COIN_PRESENT_MINIGAMES) == 0)
		{
		...
		if (mLevel == FOG_LEVEL_INDEX + 6 && mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedPuzzleMode && CountCoinByType(CoinType::COIN_PRESENT_PUZZLE_MODE) == 0)
		{
			mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
	...
	if (mApp->IsFirstTimeAdventureMode() && mLevel == NIGHT_LEVEL_INDEX + 1 && !mDroppedFirstCoin && mCurrentWave > 5)
	{
		aDropHit = 1000;
	}
	if (mApp->IsColumnLevel())
	...
	if (mApp->IsWhackAZombieLevel())
	{
		...
		int aSunChanceMax = mSunMoney > 40 ? 2800 : mSunMoney > 28 ? 3100 : mSunMoney > 16 ? 3700 : 5000;
	...
	if (mApp->IsFirstTimeAdventureMode() && mLevel == NIGHT_LEVEL_INDEX + 1)
	{
...
{
	return !mCutScene->ShouldRunUpsellBoard() && (!mApp->IsFirstTimeAdventureMode() || mLevel >= NIGHT_LEVEL_INDEX + 1);
}
...
bool Board::BungeeIsTargetingCell(int theGridX, int theGridY)
{
	...
	while (IterateZombies(aZombie))
	{
		if (!aZombie->IsDeadOrDying() && Zombie::IsBungee(aZombie->mZombieType) && aZombie->mRow == theGridY && aZombie->mTargetCol == theGridX)
		{
			return true;
		}
	}
...
Plant* Board::FindWarmPlant(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (Plant::IsWarm(aPlant->mSeedType) && !aPlant->NotOnGround() && GridInRange(theGridX, theGridY, aPlant->mPlantCol, aPlant->mRow, 1, 1))
		{
			return aPlant;
		}
	}
...
void Board::DoFwooshCol(int theCol)
{
	for (int i = 0; i < MAX_GRID_SIZE_Y; i++)
	{
		int aRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, i, 1);
		Reanimation* aOriReanim = mApp->ReanimationTryToGet(mFwooshID[i][theCol]);
		if (aOriReanim)
		{
			aOriReanim->ReanimationDie();
		}
		float aPosY = GridToPixelY(theCol, i) - 32;
		float aPosX = GridToPixelX(theCol, i) + 32;
		...
		float aScale = RandRangeFloat(0.9f, 1.1f);
		float aFlip = Rand(2) ? 1.0f : -1.0f;
		aFwoosh->OverrideScale(aFlip, aScale);

		mFwooshID[i][theCol] = mApp->ReanimationGetID(aFwoosh);
	}
	mFwooshCountDown = 100;
}
...
//0x41D8A0
void Board::KillAllZombiesInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange, bool theBurn, int theDamage, int theDamageRangeFlags, unsigned int theDamageFlags)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
	...
	if (theBurn)
	{
		...
		while (IteratePlants(aPlant))
		{
			if (GetCircleRectOverlap(theX, theY, theRadius, aPlant->GetPlantRect()))
			{
				aPlant->mWarmCounter = max(2000, aPlant->mWarmCounter);
...
void Board::KillZombieRow(int theRow, bool theBurn, int theDamage, int theDamageRangeFlags, unsigned int theDamageFlags)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
		if ((aZombie->mZombieType == ZombieType::ZOMBIE_BOSS || aZombie->mRow == theRow) && aZombie->EffectedByDamage(theDamageRangeFlags))
		{
			aZombie->TakeDamage(theDamage, theDamageFlags);
		}
	}
	...
	if (theBurn)
	{
		...
		while (IteratePlants(aPlant))
		{
			if (aPlant->mRow == theRow)
			{
				aPlant->mWarmCounter = max(1000, aPlant->mWarmCounter);
				aPlant->mChilledCounter = 0;
				aPlant->mIceTrapCounter = 0;
			}
	...
	else if (TestBit(theDamageFlags, DamageFlags::DAMAGE_FREEZE) || TestBit(theDamageFlags, DamageFlags::DAMAGE_ICE))
	{
		Zombie* aBossZombie = GetBossZombie();
		if (aBossZombie && aBossZombie->mFireballRow == theRow)
		{
			aBossZombie->BossDestroyFireballInRow(theRow);
		}
	}
}

void Board::KillZombieCol(int theCol, bool theBurn, int theDamage, int theDamageRangeFlags, unsigned int theDamageFlags)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
		...
		int aTargetCol = PixelToGridXKeepOnBoard(aZombieX, aZombie->mPosY);
		if (theCol == aTargetCol && aZombie->EffectedByDamage(theDamageRangeFlags))
		{
			aZombie->TakeDamage(theDamage, theDamageFlags);
		}
	}
...
void Board::KillZombieColFromPixel(int theX, int theY, bool theBurn, int theDamage, int theDamageRangeFlags, unsigned int theDamageFlags)
{
	Zombie* aZombie = nullptr;
	int aCol = PixelToGridX(theX, theY);
	while (IterateZombies(aZombie))
	{
		Rect aZombieRect = aZombie->GetZombieRect();
		int aZombieX = aZombieRect.mX + (aZombieRect.mWidth / 2);
		int aTargetCol = PixelToGridXKeepOnBoard(aZombieX, aZombie->mPosY);
		if (aCol == aTargetCol && aZombie->EffectedByDamage(theDamageRangeFlags))
		{
			aZombie->TakeDamage(theDamage, theDamageFlags);
		}
	}
	...
	if (theBurn)
	{
		...
		while (IterateGridItems(aGridItem))
		{
			if (aGridItem->mGridX == aCol && aGridItem->mGridItemType == GridItemType::GRIDITEM_LADDER)
			{
				aGridItem->GridItemDie();
			}
		}
...
void Board::KillPlantRow(int theRow, int theDamage)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
...
void Board::KillPlantCol(int theCol, int theDamage)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mPlantCol == theCol && !aPlant->NotOnGround())
		{
			Plant* aNormalPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
			Plant* aPumpkinPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_PUMPKIN);
...
void Board::KillPlantColFromPixel(int theX, int theY, int theDamage)
{
	int aCol = PixelToGridXKeepOnBoard(theX, theY);
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mPlantCol == aCol && !aPlant->NotOnGround())
		{
			Plant* aNormalPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
			Plant* aPumpkinPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_PUMPKIN);
...
{
	return (Plant::IsUpgrade(theSeedType) || Plant::IsPremium(theSeedType)) && mApp->IsSurvivalEndless(mApp->mGameMode);
}
...
int Board::GetCurrentPlantCost(SeedType theSeedType, SeedType theImitaterType)
{
	...
	if (PlantUsesAcceleratedPricing(theSeedType))
		aCost += CountPlantByType(theSeedType) * 4;
	if (theSeedType == SEED_COFFEEPOT)
		aCost += CountPlantByType(theSeedType) * 2;
	if (mApp->IsAcceleratedLevel())
		aCost += CountPlantByType(theSeedType);
	return aCost;
}
...
{
	return (Zombie::IsBackup(theZombieType) || Zombie::IsBobsled(theZombieType));
}

// Stage Types

bool Board::StageHasSun()
{
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_ECLIPSE)
		return false;
	return !StageIsNight();
}

bool Board::StageIsSunny()
{
	return
		mBackground == BackgroundType::BACKGROUND_HEAVEN;
}

bool Board::StageHasEclipse()
{
	return (StageIsFall() && !StageIsNight());
}

bool Board::StageHasStream()
{
	return (mBackground == BackgroundType::BACKGROUND_DAY_SPRING
		|| mBackground == BackgroundType::BACKGROUND_NIGHT_SPRING
		);
}

bool Board::StageHasTiles()
{
	return (mBackground == BackgroundType::BACKGROUND_POOL_WINTER
		|| mBackground == BackgroundType::BACKGROUND_FOG_WINTER
		|| StageHasRoof()
		);
}

bool Board::StageIsFlooded()
{
	return (mBackground == BackgroundType::BACKGROUND_POOL_SPRING
		|| mBackground == BackgroundType::BACKGROUND_FOG_SPRING
		);
}

bool Board::StageHasWater()
{
	return (StageHasPool()
		|| StageHasStream()
		|| StageIsFlooded()
		|| mBackground == BackgroundType::BACKGROUND_RESORT
		);
}

bool Board::StageHasGraveDanger()
{
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_GRAVE_DANGER)
		return true;
	return (StageHasGraveStones() && StageIsFall());
}

bool Board::StageIsDeadplants()
{
	return (mBackground == BackgroundType::BACKGROUND_DEADPLANTS);
}

bool Board::StageHasRain()
{
	return mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS || mApp->IsStormyNightLevel() || StageIsSpring();
}

bool Board::StageHasStorm()
{
	return mApp->IsStormyNightLevel() || mBackground == BackgroundType::BACKGROUND_FOG_SPRING;
}

bool Board::StageIsFall()
{
	return mBackground == BackgroundType::BACKGROUND_DAY_FALL || mBackground == BackgroundType::BACKGROUND_NIGHT_FALL ||
		mBackground == BackgroundType::BACKGROUND_POOL_FALL || mBackground == BackgroundType::BACKGROUND_FOG_FALL ||
		mBackground == BackgroundType::BACKGROUND_ROOF_FALL || mBackground == BackgroundType::BACKGROUND_MOON_FALL;
}

bool Board::StageIsWinter()
{
	return mBackground == BackgroundType::BACKGROUND_DAY_WINTER || mBackground == BackgroundType::BACKGROUND_NIGHT_WINTER ||
		mBackground == BackgroundType::BACKGROUND_POOL_WINTER || mBackground == BackgroundType::BACKGROUND_FOG_WINTER ||
		mBackground == BackgroundType::BACKGROUND_ROOF_WINTER || mBackground == BackgroundType::BACKGROUND_MOON_WINTER;
}

bool Board::StageIsSpring()
{
	return mBackground == BackgroundType::BACKGROUND_DAY_SPRING || mBackground == BackgroundType::BACKGROUND_NIGHT_SPRING ||
		mBackground == BackgroundType::BACKGROUND_POOL_SPRING || mBackground == BackgroundType::BACKGROUND_FOG_SPRING ||
		mBackground == BackgroundType::BACKGROUND_ROOF_SPRING || mBackground == BackgroundType::BACKGROUND_MOON_SPRING;
}
