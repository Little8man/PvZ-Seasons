...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY)
{
	...
	TOD_ASSERT(aZombie);
	aZombie->mPosX = theGridX * 56 + SEED_CHOOSER_ZOMBIE_XOFFSET;
	aZombie->mPosY = theGridY * 90 + SEED_CHOOSER_ZOMBIE_YOFFSET;
	if (theGridX % 2 == 1)
	{
		aZombie->mPosY += 30.0f;
	}
	...
	if (Zombie::IsZomboni(theZombieType))
	{
		aZombie->mPosY -= 10.0f;
		aZombie->mPosX -= 30.0f;
	}
	...
	if (Zombie::IsBungee(theZombieType))
	{
	...
	else if (Zombie::IsBobsled(theZombieType))
	{
...
bool CutScene::CanZombieGoInGridSpot(ZombieType theZombieType, int theGridX, int theGridY, bool theZombieGrid[5][5])
{
	...
	if (theGridX == 4 && mBoard->StageHasFog() && Zombie::IsBalloon(theZombieType))
	{
		return false;
	}

	if (Is2x2Zombie(theZombieType) || 
		Zombie::IsZomboni(theZombieType) ||
		Zombie::IsBobsled(theZombieType) ||
		Zombie::IsPole(theZombieType))
	{
		if (theGridX == 0)
		{
			return false;
...
void CutScene::PlaceStreetZombies()
{
	...
	for (int aWave = 0; aWave < mBoard->mNumWaves; aWave++)
	{
		for (int aZombieIndex = 0; aZombieIndex < MAX_ZOMBIES_IN_WAVE; aZombieIndex++)
		{
			...
			if (Zombie::IsYeti(aZombieType) && !mApp->IsStormyNightLevel())
			{
				continue;
			}
			if (Zombie::IsBobsled(aZombieType) && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_BOBSLED_BONANZA)
			{
				continue;
			}
			...
			++aTotalZombieCount;
			if (Zombie::IsBungee(aZombieType) || Zombie::IsBobsled(aZombieType))
			{
	...
	if (mApp->IsLastStandLevel())
	{
		...
		{
			if (aZombieType != ZombieType::ZOMBIE_YETI && aZombieType != ZombieType::ZOMBIE_BIGFOOT && mBoard->mZombieAllowed[aZombieType])
			{
	...
	{
		if (aZombieTypeCount[(int)aZombieType] && (Is2x2Zombie(aZombieType) || Zombie::IsZomboni(aZombieType)))
		{
...
void CutScene::StartLevelIntro()
{
	...
	// Fast Forward
	mBoard->mFastForwardButton->mBtnNoDraw = true;
	...
	if (mCrazyDaveTime <= 0 && mApp->mGameMode != GameMode::GAMEMODE_INTRO)
	{
		...
		else if (mApp->IsAdventureMode())
		{
			if (mBoard->mBackground == BackgroundType::BACKGROUND_1_DAY 
				...
				|| mBoard->mBackground == BackgroundType::BACKGROUND_DAY_FALL
				|| mBoard->mBackground == BackgroundType::BACKGROUND_NIGHT_FALL
				|| mBoard->mBackground == BackgroundType::BACKGROUND_DAY_WINTER
				|| mBoard->mBackground == BackgroundType::BACKGROUND_NIGHT_WINTER
				|| mBoard->mBackground == BackgroundType::BACKGROUND_DAY_SPRING 
				|| mBoard->mBackground == BackgroundType::BACKGROUND_NIGHT_SPRING 
				)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_HOUSE]"));
			}
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_DEADPLANTS)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_HOUSE_DEAD]"));
			}
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_HEAVEN)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_HOUSE_HEAVEN]"));
			}
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_3_POOL 
				|| mBoard->mBackground == BackgroundType::BACKGROUND_4_FOG 
				|| mBoard->mBackground == BackgroundType::BACKGROUND_POOL_FALL
				|| mBoard->mBackground == BackgroundType::BACKGROUND_FOG_FALL
				|| mBoard->mBackground == BackgroundType::BACKGROUND_POOL_WINTER
				|| mBoard->mBackground == BackgroundType::BACKGROUND_FOG_WINTER
				|| mBoard->mBackground == BackgroundType::BACKGROUND_POOL_SPRING 
				|| mBoard->mBackground == BackgroundType::BACKGROUND_FOG_SPRING
				)
			{
			...
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_RESORT)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_VACATION]"));
			}
			else
			{
				TOD_ASSERT();
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE)) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_SOMBERSANCTUARY);
		else mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_ZEN_GARDEN);
	}
	else if (mApp->mGameMode == GameMode::GAMEMODE_UPSELL)
	{
	...
	{
		if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE)) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_12FEETUNDER);
		else mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES);
	}
	...
	else if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE))
	{
		mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_LAWNDEFENSEDILEMNA);
	}
	else
	{
...
void CutScene::CancelIntro()
{
	...
	if (mCutsceneTime < mCrazyDaveTime + TimePanRightEnd)
	{
		...
		if (mBoard->mLevel == BOWLING_LEVEL)
		{
	...
	if (mCutsceneTime > mCrazyDaveTime + TimePanLeftStart || !mBoard->ChooseSeedsOnCurrentLevel())
	{
		...
		{
			mBoard->mSeedBank->Move((BOARD_WIDTH / 2) - (mBoard->mSeedBank->mWidth / 2), 0);
		}
		...
		if (mBoard->mTutorialState != TutorialState::TUTORIAL_ZEN_GARDEN_PICKUP_WATER)
		{
			mBoard->mMenuButton->mBtnNoDraw = false;
			// Fast Forward
			mBoard->mFastForwardButton->mBtnNoDraw = false;
		}
...
{
	int aPotColumns = 0;
	if (mBoard->mLevel == ROOF_LEVEL_INDEX + 1)
	{
		aPotColumns = 5;
	}
	else if (mBoard->mLevel == ROOF_LEVEL_INDEX + 2)
	{
		aPotColumns = 4;
	}
...
void CutScene::AddLilyPads()
{
	int aPadColumns = 0;
	if (mBoard->StageIsFlooded())
	{
		aPadColumns = 3;
	}

	for (int x = 0; x < aPadColumns; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
		{
...
void CutScene::AnimateBoard()
{
	...
	{
		int aSeedBankX = CalcPosition(aTimeSeedBankRightStart, aTimeSeedBankRightEnd, SEED_BANK_OFFSET_X, (BOARD_WIDTH / 2) - (mBoard->mSeedBank->mWidth / 2));
	...
	if (mSodTime > 0)
	{
		...
		if (mCutsceneTime == aTimeRollSodStart)
		{
			...
			if (mBoard->mLevel == DAY_LEVEL_INDEX + 1)
			{
			...
			else if (mBoard->mLevel == DAY_LEVEL_INDEX + 2)
			{
			...
			else if (mBoard->mLevel == DAY_LEVEL_INDEX + 4)
			{
		...
		for (int aGridY = 0; aGridY < MAX_GRID_SIZE_Y; aGridY++)
		{
			...
			if (mCutsceneTime > aTimeLawnMowerStart)
			{
				...
				if (aLawnMower)
				{
					...
					aLawnMower->mPosX = CalcPosition(aTimeLawnMowerStart, aTimeLawnMowerStart + TimeLawnMowerDuration, -80, LAWN_XMIN - 61);
				}
			}
		}
	}
	...
	{
		mApp->AddReanimation(BOARD_XCENTER, BOARD_YCENTER + 24, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_SCREEN_FADE, 0, 0), ReanimationType::REANIM_READYSETPLANT);
...
void CutScene::ShowShovel()
{
	...
	if (!mApp->IsFirstTimeAdventureMode() || mBoard->mLevel > SHOVEL_LEVEL)
	{
		mBoard->mShowShovel = true;
	}
}
...
void CutScene::UpdateZombiesWon()
{
	...
	{
		mBoard->Move(CalcPosition(LostTimePanRightStart, LostTimePanRightEnd, 0, BOARD_OFFSET_X), BOARD_OFFSET_Y);
	}
...
void CutScene::AdvanceCrazyDaveDialog(bool theJustSkipping)
{
	...
	if (!mApp->AdvanceCrazyDaveText())
	{
		...
		else
		{
			...
			{
				if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE)) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_LAWNDEFENSEDILEMNA);
				else mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_CHOOSE_YOUR_SEEDS);
			}
