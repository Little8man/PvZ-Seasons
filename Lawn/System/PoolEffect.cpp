...
void PoolEffect::PoolEffectDraw(Sexy::Graphics* g, bool theIsNight)
{
    if (!mApp->Is3DAccelerated())
    {
        if (theIsNight)
        {
            g->DrawImage(IMAGE_POOL_NIGHT, LAWN_XMIN - 6, LAWN_YMIN + 198);
        }
        else
        {
            g->DrawImage(IMAGE_POOL, LAWN_XMIN - 6, LAWN_YMIN + 198);
        }
        return;
    }
        ...
        for (int y = 0; y < 5; y++)
        {
            for (int aLayer = 0; aLayer < 3; aLayer++)
            {
                ...
                for (int aVertIndex = 0; aVertIndex < 6; aVertIndex++, pVert++)
                {
                    ...
                    if (aLayer == 2)
                    {
                        pVert->x = (704.0f / 15.0f) * aIndexX + 5.0f + LAWN_XMIN;
                        pVert->y = 30.0f * aIndexY + 208.0f + LAWN_YMIN;
                    ...
                    else
                    {
                        ...
                        pVert->x = aIndexX * aGridSquareX - 5 + LAWN_XMIN;
                        pVert->y = aIndexY * aGridSquareY + 199 + LAWN_YMIN;
