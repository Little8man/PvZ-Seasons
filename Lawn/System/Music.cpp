...
void Music::MusicTitleScreenInit()
{
	...
	LoadSong(MusicFile::MUSIC_FILE_GIGA_ZOMBOTANY, "sounds\\giga\\gigamusic_zombotany.mp3");
	if (mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE)) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_ZOMBOTANY);
	else MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_TITLE_CRAZY_DAVE_MAIN_THEME);
}
...
void Music::MusicInit()
{
	...
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_SEASONS_HARDLYHOPING, "sounds\\seasons\\seasonsmusic_hardlyhoping.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_MARCHWEED, "sounds\\giga\\gigamusic_marchweed.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_STARJELLY, "sounds\\giga\\gigamusic_starjelly.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_12FEETUNDER, "sounds\\giga\\gigamusic_12feetunder.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_BLINDFATE, "sounds\\giga\\gigamusic_blindfate.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_SCOFFINGDOWNSHINGLES, "sounds\\giga\\gigamusic_scoffingdownshingles.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_LAWNDEFENSEDILEMNA, "sounds\\giga\\gigamusic_lawndefensedilemna.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_SOMBERSANCTUARY, "sounds\\giga\\gigamusic_sombersanctuary.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_RUSHINGTOYOURHEAD, "sounds\\giga\\gigamusic_rushingtoyourhead.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_PRANKSTERSPARADIGM, "sounds\\giga\\gigamusic_marchweed.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_FORBIDDENFRUIT, "sounds\\giga\\gigamusic_forbiddenfruit.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_GIGA_ARCANEBRAINIAC, "sounds\\giga\\gigamusic_arcanebrainiac.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_DEADPLANTS_ENGULFED, "sounds\\deadplantsmusic_engulfed.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_HEAVEN_GRISLYFAITH, "sounds\\heavenmusic_grislyfaith.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
	LoadSong(MusicFile::MUSIC_FILE_MINECRAFT_CUBELAND, "sounds\\minecraftmusic_cubeland.mp3");
	mApp->mCompletedLoadingThreadTasks += 3500;
...
void Music::PlayMusic(MusicTune theMusicTune, int theOffset, int theDrumsOffset)
{
	...
	case MusicTune::MUSIC_TUNE_CREDITS_ZOMBIES_ON_YOUR_LAWN:
		...
		break;
		// Seasons Music
	case MusicTune::MUSIC_TUNE_SEASONS_HARDLYHOPING:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_SEASONS_HARDLYHOPING;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	// Giga Music
	case MusicTune::MUSIC_TUNE_GIGA_MARCHWEED:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_MARCHWEED;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_STARJELLY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_STARJELLY;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_12FEETUNDER:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_12FEETUNDER;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_BLINDFATE:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_BLINDFATE;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_SCOFFINGDOWNSHINGLES:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_SCOFFINGDOWNSHINGLES;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_LAWNDEFENSEDILEMNA:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_LAWNDEFENSEDILEMNA;
		if (theOffset == -1) theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_ZOMBOTANY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_ZOMBOTANY;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_SOMBERSANCTUARY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_SOMBERSANCTUARY;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_RUSHINGTOYOURHEAD:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_RUSHINGTOYOURHEAD;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_PRANKSTERSPARADIGM:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_PRANKSTERSPARADIGM;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_FORBIDDENFRUIT:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_FORBIDDENFRUIT;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_GIGA_ARCANEBRAINIAC:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GIGA_ARCANEBRAINIAC;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_DEADPLANTS_ENGULFED:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_DEADPLANTS_ENGULFED;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
		// Minigames
	case MusicTune::MUSIC_TUNE_HEAVEN_GRISLYFAITH:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_HEAVEN_GRISLYFAITH;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;

	case MusicTune::MUSIC_TUNE_MINECRAFT_CUBELAND:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_MINECRAFT_CUBELAND;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
...
void Music::StartGameMusic()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_SOMBERSANCTUARY) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_ZEN_GARDEN);
	else if (mApp->IsFinalBossLevel())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_ARCANEBRAINIAC) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_FINAL_BOSS_BRAINIAC_MANIAC);
		...
		StopAllMusic();
	else if (mApp->IsMiniBossLevel() || mApp->IsColumnLevel())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_FORBIDDENFRUIT) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_CONVEYER);
	else if (mApp->IsMinecraftLevel())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_MINECRAFT_CUBELAND);
	else if (mApp->IsScaryPotterLevel() || mApp->IsIZombieLevel())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_RUSHINGTOYOURHEAD) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_PUZZLE_CEREBRAWL);
	else if (mApp->mBoard->StageIsDeadplants())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_DEADPLANTS_ENGULFED);
	else if (mApp->mBoard->StageHasFog())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_12FEETUNDER) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_FOG_RIGORMORMIST);
	else if (mApp->mBoard->StageIsNight())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_STARJELLY) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_NIGHT_MOONGRAINS);
	else if (mApp->mBoard->StageHasEclipse())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_SEASONS_HARDLYHOPING);
	else if (mApp->mBoard->StageHasPool() || mApp->mBoard->StageIsFlooded())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_12FEETUNDER) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES);
	else if (mApp->mBoard->StageHasRoof())
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_SCOFFINGDOWNSHINGLES) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_ROOF_GRAZETHEROOF);
	else if (mApp->mBoard->StageIsSunny())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_HEAVEN_GRISLYFAITH);
	else
		mApp->GetConfigCheckbox(true, CONFIG_CHECKBOX_GIGA_MODE) ? MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GIGA_MARCHWEED) : MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_DAY_GRASSWALK);
}
