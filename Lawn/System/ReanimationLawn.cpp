...
void ReanimatorCache::GetPlantImageSize(SeedType theSeedType, int& theOffsetX, int& theOffsetY, int& theWidth, int& theHeight)
{
	...
	if (theSeedType == SeedType::SEED_TALLNUT || theSeedType == SeedType::SEED_VINENUT)
	{
		theOffsetY = -40;
		theHeight += 40;
	}
...
MemoryImage* ReanimatorCache::MakeCachedPlantFrame(SeedType theSeedType, DrawVariation theDrawVariation)
{
	...
	if (theSeedType == SeedType::SEED_POTATOMINE || theSeedType == SeedType::SEED_POPCORN)
	{
		aMemoryGraphics.mScaleX = 0.85f;
		aMemoryGraphics.mScaleY = 0.85f;
		DrawReanimatorFrame(&aMemoryGraphics, -aOffsetX + 12, -aOffsetY + 12, aPlantDef.mReanimationType, "anim_armed", theDrawVariation);
	}
	else if (theSeedType == SeedType::SEED_INSTANT_COFFEE || theSeedType == SeedType::SEED_HOTPOTATO)
	{
		aMemoryGraphics.mScaleX = 0.8f;
		aMemoryGraphics.mScaleY = 0.8f;
		DrawReanimatorFrame(&aMemoryGraphics, -aOffsetX + 12, -aOffsetY + 12, aPlantDef.mReanimationType, "anim_idle", theDrawVariation);
	}
	else if (theSeedType == SeedType::SEED_HAZYNUT)
	{
		aMemoryGraphics.SetColorizeImages(true);
		aMemoryGraphics.SetColor(Color(255, 64, 255));
		DrawReanimatorFrame(&aMemoryGraphics, -aOffsetX, -aOffsetY, aPlantDef.mReanimationType, "anim_idle", theDrawVariation);
	}
	else if (theSeedType == SeedType::SEED_FROSTACHIO)
	{
		aMemoryGraphics.SetColorizeImages(true);
		aMemoryGraphics.SetColor(Color(64, 255, 255));
		DrawReanimatorFrame(&aMemoryGraphics, -aOffsetX, -aOffsetY, aPlantDef.mReanimationType, "anim_idle", theDrawVariation);
	}
	else
	{
		...
		if (Plant::IsPeashooter(theSeedType) ||
...
MemoryImage* ReanimatorCache::MakeCachedZombieFrame(ZombieType theZombieType)
{
	...
	else
	{
		...
		if (Zombie::IsPogo(theZombieType))
		{
		...
		else if (Zombie::IsPole(theZombieType))
		{
			aTrackName = "anim_walk";
		}
		else if (Zombie::IsGargantuar(theZombieType))
		{
			aPosY = 60.0f;
		}
