...
void LawnMower::LawnMowerInitialize(int theRow)
{
    ...
    else if ((mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL || mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_STREAM) && mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_POOL_CLEANER])
    {
...
void LawnMower::Update()
{
    ...
    {
        mRollingInCounter++;
        mPosX = TodAnimateCurveFloat(0, 100, mRollingInCounter, -160.0f, LAWN_XMIN - 61, TodCurves::CURVE_EASE_IN_OUT);
    ...
    while (mBoard->IterateZombies(aZombie))
    {
            ...
            aZombie->EffectedByDamage(127U))
        {
            ...
            if (aOverlap > (Zombie::IsBalloon(aZombie->mZombieType) ? 20 : 0))
            {
                // 蹦极僵尸或已死亡的僵尸不能主动触发小推车
                if (mMowerState != LawnMowerState::MOWER_READY || (!Zombie::IsBungee(aZombie->mZombieType) && aZombie->mHasHead))
                {
                    MowZombie(aZombie);
                }
            }
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_LAWN && ((mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL && mPosX > 10.0f + LAWN_XMIN) || (mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_STREAM && mPosX > 250.0f + LAWN_XMIN)))
    {
