...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    if (IsSun() || mType == COIN_SHADOW_SUN)
    {
        float aPosX = mWidth * 0.5f;
        float aPosY = mHeight * 0.5f;
        mValue = GetSunValue(mType);
        ReanimationType aReanimationType = mType == COIN_SHADOW_SUN ? ReanimationType::REANIM_SHADOWSUN : ReanimationType::REANIM_SUN;
...
bool Coin::IsSun()
{
    return mType == CoinType::COIN_SUN || mType == CoinType::COIN_SMALLSUN || mType == CoinType::COIN_LARGESUN || mType == CoinType::COIN_CUSTOMSUN;
}
...
void Coin::ScoreCoin()
{
    ...
    if (IsSun())
    {
        int aSunValue = mValue;
        mBoard->AddSunMoney(aSunValue);
    }
    else if (mType == COIN_SHADOW_SUN)
    {
        int aShadowSunValue = mValue;
        mBoard->AddShadowSunMoney(aShadowSunValue);
    }
    else if (IsMoney())
    {
        int aCoinValue = mValue;
...
void Coin::UpdateFall()
{
    ...
    else
    {
        ...
        if (!mApp->IsLastStandLevel() || mBoard == nullptr ||
...
Color Coin::GetColor()
{
    if ((IsSun() || mType == COIN_SHADOW_SUN || IsMoney()) && mIsBeingCollected)
    {
...
void Coin::Draw(Graphics* g)
{
    ...
    else if (IsSun() || mType == COIN_SHADOW_SUN)
    {
        return;
    }
...
void Coin::Collect()
{
    ...
    if (IsLevelAward())
    {
        ...
        else if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == SHOVEL_LEVEL)
        {
            mApp->PlaySample(SOUND_SHOVEL);
        }
        else if (mApp->IsFirstTimeAdventureMode() && (mBoard->mLevel % LEVELS_PER_AREA == LEVEL_MID_AREA - 1))
        {
    ...
    if (IsSun() && mBoard && !mBoard->HasConveyorBeltSeedBank())
    {
        for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
        {
            ...
            if (aSunProfit >= 0 && aSunProfit < mValue)
            {
...
{
    return theCoinType == CoinType::COIN_SMALLSUN ? 1 : theCoinType == CoinType::COIN_LARGESUN ? 4 : 2;
}
...
void Coin::PlayCollectSound()
{
    ...
    if (mType == COIN_SHADOW_SUN)
    {
        mApp->PlayFoley(FoleyType::FOLEY_SHADOW_SUN);
        return;
    }
...
void Coin::MouseDown(int x, int y, int theClickCount)
{
    ...
    if (true && theClickCount >= 0 && !mIsBeingCollected)
    {
        PlayCollectSound();
        Collect();

        if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == DAY_LEVEL_INDEX + 1)
        {
...
bool Coin::CoinGetsBouncyArrow()
{
    ...
    if (mType == CoinType::COIN_SILVER || mType == CoinType::COIN_GOLD)
    {
        if (mApp->IsFirstTimeAdventureMode() && mBoard && mBoard->mLevel == NIGHT_LEVEL_INDEX + 1 && !mBoard->mDroppedFirstCoin)
        {
            return true;
        }
    }
