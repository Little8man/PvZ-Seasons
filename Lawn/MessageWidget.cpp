...
//0x459710
void MessageWidget::DrawReanimatedText(Graphics* g, Font* theFont, const Color& theColor, float thePosY, float thePosX)
{
	int aLabelLen = strlen(mLabel);
	for (int aPos = 0; aPos < aLabelLen; aPos++)
	{
		...
		aTransform.mTransX += aTextReanim->mOverlayMatrix.m02 + thePosX + BOARD_XCENTER - 400;
		aTransform.mTransY += aTextReanim->mOverlayMatrix.m12 + thePosY + BOARD_YCENTER - 300;
...
void MessageWidget::Draw(Graphics* g)
{
	...
	if (mReanimType != ReanimationType::REANIM_NONE)
	{
		if (aFont == Sexy::FONT_CONTINUUMBOLD14)
		{
			DrawReanimatedText(g, Sexy::FONT_CONTINUUMBOLD14OUTLINE, Color::Black, aPosY, aPosX);
		}
		DrawReanimatedText(g, aFont, aColor, aPosY, aPosX);
	}
	else
	{
		if (aMinAlpha != 255)
		...
		if (aRectHeight > 0)
		{
			...
			Rect aRect(0, aPosY + BOARD_YCENTER, BOARD_WIDTH, aRectHeight);
		...
		{
			Rect aRect(aPosX - mApp->mBoard->mX, aPosY + BOARD_YCENTER - aFont->mAscent, BOARD_WIDTH, BOARD_HEIGHT);
			if (aOutlineFont)
			{
		...
		if (mMessageStyle == MessageStyle::MESSAGE_STYLE_HOUSE_NAME)
		{
			...
			if (aSubStr.size() > 0)
			{
				TodDrawString(
					g,
					aSubStr,
					aPosX + BOARD_XCENTER,// - mApp->mBoard->mX, 
					aPosY + 26,
