...
#include "Lawn/Widget/ConfigScreen.h"
...
LawnApp::~LawnApp()
{
	...
	if (mConfigScreen)
	{
		mWidgetManager->RemoveWidget(mConfigScreen);
		delete mConfigScreen;
	}
...
void LawnApp::ShowConfigScreen(ConfigPage thePage)
{
	mGameScene = GameScenes::SCENE_CONFIG;
	mConfigScreen = new ConfigScreen(this, thePage);
	mConfigScreen->Resize(0, 0, mWidth, mHeight);
	mWidgetManager->AddWidget(mConfigScreen);
	mWidgetManager->BringToBack(mConfigScreen);
	mWidgetManager->SetFocus(mConfigScreen);
}

void LawnApp::KillConfigScreen()
{
	if (mConfigScreen)
	{
		mWidgetManager->RemoveWidget(mConfigScreen);
		SafeDeleteWidget(mConfigScreen);
		mConfigScreen = nullptr;
	}
}
...
void LawnApp::Init()
{
	...
	// U53RDV
	mDevModeCheck = new TypingCheck("u53rdv");
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	...
	if (IsAdventureMode())
	{
		...
		if (!HasFinishedAdventure() && mBoard->mLevel == FOG_LEVEL_INDEX + LEVEL_MID_AREA - 1)
		{
...
{
	return IsSurvivalNormal(mGameMode)
		|| IsSurvivalHard(mGameMode)
		|| IsSurvivalEndless(mGameMode)
		;
}
...
bool LawnApp::IsSurvivalNormal(GameMode theGameMode)
{
	return (theGameMode >= GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1 && theGameMode <= GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_5)
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON
		|| (theGameMode >= GameMode::GAMEMODE_SURVIVAL_NORMAL_DAY_FALL && theGameMode <= GameMode::GAMEMODE_SURVIVAL_NORMAL_MOON_SPRING)
		;
}
...
bool LawnApp::IsSurvivalHard(GameMode theGameMode)
{
	return (theGameMode >= GameMode::GAMEMODE_SURVIVAL_HARD_STAGE_1 && theGameMode <= GameMode::GAMEMODE_SURVIVAL_HARD_STAGE_5)
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_HARD_MOON
		|| (theGameMode >= GameMode::GAMEMODE_SURVIVAL_HARD_DAY_FALL && theGameMode <= GameMode::GAMEMODE_SURVIVAL_HARD_MOON_SPRING)
		;
}
...
bool LawnApp::IsSurvivalEndless(GameMode theGameMode)
{
	return (theGameMode >= GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_1 && theGameMode <= GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_5)
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON
		|| (theGameMode >= GameMode::GAMEMODE_SURVIVAL_ENDLESS_DAY_FALL && theGameMode <= GameMode::GAMEMODE_SURVIVAL_ENDLESS_MOON_SPRING)
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_ZOMBOTANY
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_WALLNUT_BOWLING
		|| theGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_CHAOS
		;
}
...
bool LawnApp::IsWallnutBowlingLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING || mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 || mGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_WALLNUT_BOWLING)
		return true;

	return IsAdventureMode() && mPlayerInfo->mLevel == BOWLING_LEVEL;
}

bool LawnApp::IsZombotanyLevel()
{
	if (mBoard == nullptr)
		return false;

	return mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS 
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_2 
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBOWLING
		|| mGameMode == GameMode::GAMEMODE_SURVIVAL_ENDLESS_ZOMBOTANY
		;
}
...
bool LawnApp::IsLastStandLevel()
{
	...
	return mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_ICE
		;
}

bool LawnApp::IsMinecraftLevel()
{
	if (mBoard == nullptr)
		return false;

	return mGameMode == GameMode::GAMEMODE_CHALLENGE_CUBELAND;
}

bool LawnApp::IsSkipAreaLevel()
{
	if (mBoard == nullptr)
		return false;

	return mGameMode == GameMode::GAMEMODE_CHALLENGE_DAY
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_NIGHT
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_POOL
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_FOG
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_ROOF;
}

bool LawnApp::IsColumnLevel()
{
	if (mBoard == nullptr)
		return false;
	...
	return IsAdventureMode() && mPlayerInfo->mLevel == COLUMN_LEVEL;
}

bool LawnApp::IsPortalLevel()
{
	if (mBoard == nullptr)
		return false;

	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT)
		return true;

	return IsAdventureMode() && mPlayerInfo->mLevel == PORTAL_LEVEL;
}
...
bool LawnApp::IsWhackAZombieLevel()
{
	...
	return IsAdventureMode() && mPlayerInfo->mLevel == MALLET_LEVEL;
}

//0x4538C0
bool LawnApp::IsLittleTroubleLevel()
{
	return (mBoard && (mGameMode == GameMode::GAMEMODE_CHALLENGE_LITTLE_TROUBLE || (mGameMode == GameMode::GAMEMODE_ADVENTURE && mPlayerInfo->mLevel == LITTLE_LEVEL)));
}

bool LawnApp::IsDeadplantsLevel()
{
	return (mBoard && (mGameMode == GameMode::GAMEMODE_CHALLENGE_DEADPLANTS || GetConfigCheckbox(true, CONFIG_CHECKBOX_DEADPLANTS_MODE)));
}

bool LawnApp::IsAcceleratedLevel()
{
	return (mBoard && (mGameMode == GameMode::GAMEMODE_CHALLENGE_LOOKUPPVZINFLATION));
}
...
bool LawnApp::IsScaryPotterLevel()
{
	...
	return IsAdventureMode() && mPlayerInfo->mLevel == VASE_LEVEL;
}
...
bool LawnApp::IsStormyNightLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_STORMY_NIGHT
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_FOG)
		return true;

	return IsAdventureMode() && mPlayerInfo->mLevel == STORMY_LEVEL;
}
...
bool LawnApp::IsBungeeBlitzLevel()
{
	...
	return IsAdventureMode() && mPlayerInfo->mLevel == BUNGEE_LEVEL;
}
...
bool LawnApp::IsMiniBossLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_DAY
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_NIGHT
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_POOL
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_ROOF
		|| mGameMode == GameMode::GAMEMODE_CHALLENGE_MINIBOSS_FOG
		)
		return true;

	return
		(IsAdventureMode() && mPlayerInfo->mLevel % 10 == 0 && mPlayerInfo->mLevel != FINAL_LEVEL);
}
...
bool LawnApp::IsFinalBossLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_FINAL_BOSS || mGameMode == GameMode::GAMEMODE_CHALLENGE_WHAT)
		return true;

	return IsAdventureMode() && (mPlayerInfo->mLevel == FINAL_LEVEL || mPlayerInfo->mLevel == ZOMBOSS_LEVEL);
}
...
bool LawnApp::IsNight()
{
	...
	return (mPlayerInfo->mLevel > NIGHT_LEVEL_INDEX && mPlayerInfo->mLevel <= NIGHT_LEVEL_INDEX + LEVELS_PER_AREA) || 
			(mPlayerInfo->mLevel > FOG_LEVEL_INDEX && mPlayerInfo->mLevel <= FOG_LEVEL_INDEX + LEVELS_PER_AREA) || 
			(mPlayerInfo->mLevel > MOON_LEVEL_INDEX && mPlayerInfo->mLevel <= MOON_LEVEL_INDEX + LEVELS_PER_AREA) || 
			mPlayerInfo->mLevel == FINAL_LEVEL;
}
...
SeedType LawnApp::GetAwardSeedForLevel(int theLevel)
{
	...
	if (aSeedsHasGot > NUM_LEVEL_SEEDS)
	{
		aSeedsHasGot = NUM_LEVEL_SEEDS;
	}
...
bool LawnApp::CanShowAlmanac()
{
	...
	return HasFinishedAdventure() || mPlayerInfo->mLevel >= NIGHT_LEVEL_INDEX + LEVEL_MID_AREA;
}
...
bool LawnApp::CanShowStore()
{
	...
	return HasFinishedAdventure() || mPlayerInfo->mHasSeenUpsell || mPlayerInfo->mLevel >= POOL_LEVEL_INDEX + LEVEL_MID_AREA;
}
...
bool LawnApp::CanShowZenGarden()
{
	...
	return HasFinishedAdventure() || mPlayerInfo->mLevel >= FOG_LEVEL_INDEX + LEVEL_MID_AREA;
}
...
void LawnApp::PreloadForUser()
{
	...
	mCompletedLoadingThreadTasks += 68;
	// Newstuffs
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_SHADOWSUN, true);
	mCompletedLoadingThreadTasks += 68;
	...
	if (mPlayerInfo)
	{
		...
		for (ZombieType i = ZombieType::ZOMBIE_NORMAL; i < ZombieType::NUM_ZOMBIE_TYPES;i = (ZombieType)((int)i + 1))
		{
			...
			if (i == ZombieType::ZOMBIE_BOSS || Zombie::IsCatapult(i) || Zombie::IsGargantuar(i) ||
				Zombie::IsDigger(i) || Zombie::IsZomboni(i))
			{
				continue;
			}
...
{
	int aMultiplier = gLawnApp->GetConfigCheckbox(false, CONFIG_CHECKBOX_COIN_AMOUNT) ? 10 : 1;
	int aValue = theAmount * aMultiplier;
	if (aValue > 999999)
	{
...
SexyString LawnGetCurrentLevelName()
{
	...
	if (gLawnApp->mGameScene == GameScenes::SCENE_CONFIG)
	{
		return _S("Config Screen");
	}
...
bool LawnApp::GetConfigCheckbox(bool theSaveBoard, int theConfigCheckbox)
{
	if (mBoard && theSaveBoard)
		return mBoard->mConfigCheckbox[theConfigCheckbox];
	else if (mPlayerInfo)
		return mPlayerInfo->mConfigCheckbox[theConfigCheckbox];
	return false;
}
